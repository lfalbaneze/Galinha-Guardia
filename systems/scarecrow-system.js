...
const ScarecrowSystem = (() => {
    ...
    function location(layout) {
        if (!layout)
            return null;
        if (places.has(layout))
            return places.get(layout);
        const area = layout.areas.find(a => a.id === 'granja');
        if (!area) {
            places.set(layout, null);
            return null;
        }
        const corn = layout.plots?.find(p => p.kind === 'corn') || area;
        const preferred = { x: corn.x + corn.w * .7, y: corn.y + corn.h + 142 };
        const occupied = [...FarmArt.getProps(layout).filter(p => p.type !== 'corn').map(p => FarmDetails.shape(p)),
        ...
        for (const margin of [90, 300]) {
            for (let y = Math.max(158, area.y - margin); y < Math.min(WORLD.height - 50, area.y + area.h + margin); y += 24)
                for (let x = Math.max(70, area.x - margin); x < Math.min(WORLD.width - 70, area.x + area.w + margin); x += 24) {
                    ...
                    if (occupied.some(r => overlap(box, r, 10)))
                        continue;
    ...
    function returnHome(s) {
        ...
        for (const [i, b] of s.birds.entries()) {
            b.from = { x: b.x, y: b.y, z: b.z };
            b.target = { x: s.x + perches[i].x, y: s.y, z: perches[i].z };
            b.delay = i * .18;
            b.duration = 2.5;
            b.progress = 0;
            b.left = b.target.x < b.x;
    ...
    function update(game, dt) {
        ...
        if (!s || game.phase !== 'playing' || game.lake?.active || !Number.isFinite(dt) || dt <= 0)
            return;
        const step = Math.min(dt, .1);
        s.clock += step;
        if ((s.mode === 'perched' || s.mode === 'returning') && near(game, game.entities.chicken.sprinting ? 195 : 145))
            flee(game);
        if (s.mode === 'perched')
            return;
        if (s.mode === 'away') {
        ...
        for (const b of s.birds) {
            ...
            b.x = lerp(b.from.x, b.target.x, ease);
            b.y = lerp(b.from.y, b.target.y, ease);
    ...
    function drawPost(game) {
        ...
        if (s.mode !== 'away')
            for (const [i, b] of s.birds.entries())
                if (!b.flying)
                    ScarecrowArt.drawCrow(ctx, b, i, s.clock, camera);
    }
    function drawFlying(game) {
        const s = game.scarecrow;
        if (!s || s.mode === 'away')
            return;
        for (const [i, b] of s.birds.entries())
    ...
    function drawShadows(game) {
        ...
        if (!s || s.mode === 'away')
            return;
        ctx.save();
        ctx.fillStyle = '#293624';
        for (const b of s.birds)
            if (b.flying && b.opacity > 0) {
                ctx.globalAlpha = .14 * b.opacity;
                ctx.beginPath();
                ctx.ellipse(b.x - camera.x + (camera.shakeX || 0), b.y - camera.y + (camera.shakeY || 0), 10, 3, 0, 0, Math.PI * 2);
                ctx.fill();
            }
        ctx.restore();
    }
