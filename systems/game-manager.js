...
const GameManager = (() => {
  ...
  function rescue(game, animal) {
    ...
    if (chick) SkinSystem.record(game);
    return true;
  }
  function win(game) {
  ...
  function read() {
    try {
      ...
      if (!["playing", "win_cutscene", "won"].includes(data.phase)) return null;
      ...
      if (data.phase !== "playing" && data.rescuedIds.length !== WORLD.targetRescues) return null;
      if (data.version >= 3) {
