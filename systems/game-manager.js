...
const GameManager = (() => {
    ...
    function rewardRescueTime(game, chick = false) {
        ...
        if (chick && game.settings.chickCombo) {
            ...
            if (friends % (game.settings.friendTimeEvery || 1) !== 0)
                seconds = 0;
        }
        if (seconds > 0) {
    ...
    function restore(game, data) {
        ...
        const maxChickTime = Array.from({ length: game.rescuedChicks }, (_, i) => (game.settings.chickTime || 0) * (game.settings.chickCombo ? Math.min(10, 2 ** i) : 1)).reduce((sum, n) => sum + n, 0);
        ...
        game.timeRemaining = game.settings.timeLimit ? bounded(savedTime, 0, finished || game.legacyTimer ? Math.max(600, allowance) : allowance, game.settings.timeLimit) : null;
        game.timeBonus = bounded(data.timeBonus, 0, Number.MAX_SAFE_INTEGER);
        game.timeBonusRate = bounded(data.timeBonusRate, 0, Number.MAX_SAFE_INTEGER, data.version < 5 && game.timeBonus ? data.difficulty === 'hardcore' ? 10000 : 1000 : timeScoreRate(game));
        game.chickCombo = { count: 0, remaining: 0 };
        ...
        for (const [index, chick] of game.entities.chicks.entries()) {
            ...
            if (newGeography)
                chick.lastSeen = chick.discovered ? { x: chick.x, y: chick.y } : null;
    ...
    function update(game, dt) {
        ...
        if (game.chickCombo.remaining > 0) {
            ...
            if (game.chickCombo.remaining <= 1e-8)
                game.chickCombo = { count: 0, remaining: 0 };
        }
