...
const SwimmingSystem = (() => {
    const FEET = 14;
    function depthAt(game, feet) {
        if (game.phase === 'win_cutscene' || game.phase === 'won')
            return 0;
        const pond = WORLD.layout.structures.pond;
        if (!pond || pond.w <= 0 || pond.h <= 0)
            return 0;
        if (game.lake?.completed) {
            const b = LakeChallenge.bridge();
            if (feet.x >= b.x && feet.x <= b.x + b.w && feet.y >= b.y && feet.y <= b.y + b.h)
                return 0;
        }
    ...
    function profile(game) {
        const c = game.entities.chicken, species = CharacterArt.appearances[c.skin || 'classic']?.species;
    ...
    function draw(game) {
        const swim = profile(game), c = game.entities.chicken;
        if (swim.depth <= 0 || c.hidden)
            return false;
        ...
        const pond = WORLD.layout.structures.pond;
        ctx.beginPath();
        ctx.ellipse(worldX(pond.x + pond.w / 2), worldY(pond.y + pond.h / 2), pond.w * .47, pond.h * .47, 0, 0, Math.PI * 2);
        ctx.clip();
        ctx.fillStyle = '#246f7933';
        ctx.beginPath();
        ctx.ellipse(p.x, waterY + 3, rx + 3, 9, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.strokeStyle = '#d7f1d8';
        ctx.lineWidth = 1.5;
        ...
        for (let i = 0; i < (c.moving ? 3 : 1); i++) {
            ...
            ctx.globalAlpha *= .7;
            ctx.beginPath();
            ctx.ellipse(p.x + vx, waterY + vy, rx + 4 + step * .3, 7 + step * .2, 0, .05, Math.PI - .05);
            ctx.stroke();
        }
        if (swim.native && c.moving) {
            ctx.globalAlpha = .35 * amount;
            ctx.fillStyle = '#eeb15d';
            const kick = reduced ? 0 : Math.sin(clock * 11) * 3;
            ctx.fillRect(p.x - 12, waterY + 2 + kick, 5, 3);
            ctx.fillRect(p.x + 7, waterY + 2 - kick, 5, 3);
        }
        ctx.restore();
        if (!swim.native) {
            ctx.save();
            ctx.globalAlpha *= amount;
            CharacterArt.drawFloat(ctx, p.x, ringY, rx * 2, false);
            ctx.restore();
        }
        ctx.save();
        ctx.beginPath();
        ctx.rect(p.x - 100, p.y - 160, 200, waterY + 3 - (p.y - 160));
        ctx.clip();
        CharacterArt.draw(ctx, 'chicken', p.x, p.y + amount * sink + bob, { skin: c.skin, direction: CharacterArt.heading(c), moving: c.moving, anim: c.anim * .5, shadow: false });
        ctx.restore();
        if (!swim.native) {
            ctx.save();
            ctx.globalAlpha *= amount;
            CharacterArt.drawFloat(ctx, p.x, ringY, rx * 2, true);
            ctx.restore();
        }
        else {
            ctx.globalAlpha *= amount;
            ctx.strokeStyle = '#dcf4de';
            ctx.lineWidth = 2;
            ctx.beginPath();
            ctx.ellipse(p.x, waterY + 2, Math.min(23, rx - 3), 4, 0, 0, Math.PI);
            ctx.stroke();
        }
        ctx.restore();
        return true;
    ...
    function drawWater(game) {
        ...
        for (let i = 0; i < 9; i++) {
            ...
            if (game.lake?.completed && depthAt(game, p) === 0)
                continue;
            ctx.globalAlpha = .14 + (Math.sin(clock * 1.5 + i) + 1) * .12;
            ctx.fillStyle = '#daf4dc';
