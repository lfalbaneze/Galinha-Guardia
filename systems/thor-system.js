...
const ThorSystem = (() => {
    ...
    function update(game, dt) {
        ...
        if (!dog) {
            ...
            if (visit.nextIn <= 0 && !arrive(game))
                visit.nextIn = 2; // Retry inaccessible terrain, without rerolling the event.
            return;
        }
        ...
        if (dog.mode === 'enter') {
            ...
            if ((distance(dog, game.entities.chicken) < 85 && DetectionSystem.hasLineOfSight(getHitbox(dog), getHitbox(game.entities.chicken))) || dog.timer <= 0)
                greet(game, dog);
        }
