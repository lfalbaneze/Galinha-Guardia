...
const EnvironmentSystem = (() => {
    ...
    const ellipse = (p, x, y, rx, ry) => ((p.x - x) / rx) ** 2 + ((p.y - y) / ry) ** 2 <= 1;
    const reduced = () => typeof InterfaceMotion !== 'undefined' && InterfaceMotion.reduced;
    const feet = (p) => ({ x: p.x, y: p.y + FEET });
    function initialize(game) {
        const scene = { layout: WORLD.layout, grid: new Map(), reactions: new Map(), marks: [], particles: [],
            stride: 0, wet: 0, muddy: 0, sound: 0, noise: 0, serial: 0, last: null, surface: 'grass' };
        const props = [...FarmArt.getProps(scene.layout), ...(scene.layout.decorations || []).filter(p => p.type !== 'corn')];
        for (const prop of props) {
            if (!soft.has(prop.type))
                continue;
            ...
            const plant = { key: key(prop), prop, root,
            ...
            for (let y = Math.floor((root.y - plant.ry) / CELL); y <= Math.floor((root.y + plant.ry) / CELL); y++)
                for (let x = Math.floor((root.x - plant.rx) / CELL); x <= Math.floor((root.x + plant.rx) / CELL); x++) {
                    ...
                    list.push(plant);
                    scene.grid.set(cell, list);
                }
        }
        scenes.set(game, scene);
    }
    function sceneFor(game) {
        if (scenes.get(game)?.layout !== WORLD.layout)
            initialize(game);
        return scenes.get(game);
    }
    function surfaceAt(game, p) {
        const layout = WORLD.layout, pond = layout.structures.pond;
        if (game.lake?.completed && inside(p, LakeChallenge.bridge()))
            return 'bridge';
        if (pond && ellipse(p, pond.x + pond.w / 2, pond.y + pond.h / 2, pond.w * .47, pond.h * .47))
            return 'water';
        for (const h of layout.habitats || []) {
            if (h.kind !== 'water' && h.water && ellipse(p, h.water.x, h.water.y, 19, 10))
                return 'water';
        ...
        if ([layout.paths, layout.lanes || [], layout.clearings || []].some(list => list.some(r => inside(p, r))))
            return 'dirt';
        return 'grass';
    }
    function movementScale(game) {
    ...
    function addMark(scene, p, kind, size, heading = 0) {
        scene.marks.push({ ...p, kind, size, heading, age: 0, life: kind === 'ripple' ? 1.15 : 3.4 });
        if (scene.marks.length > 64)
            scene.marks.shift();
    }
    function scatter(scene, p, count, color, leaf = false, height = 0, petal = false) {
        if (reduced())
        ...
        if (scene.particles.length > 96)
            scene.particles.splice(0, scene.particles.length - 96);
    }
    function contact(scene, p, dx, force) {
        let rustle = false;
        for (const plant of scene.grid.get(`${Math.floor(p.x / CELL)}:${Math.floor(p.y / CELL)}`) || []) {
            ...
            const reaction = old || { plant, bend, age: 0, shed: 0 };
            reaction.bend = old ? old.bend * .35 + bend * .65 : bend;
            reaction.age = 0;
            if (reaction.shed <= 0 && !['grass', 'clover'].includes(type)) {
                if (type === 'crop') {
                    if (force > 1)
                        scatter(scene, plant.root, 1, '#aec95b', true, 13);
                    reaction.shed = .9;
                }
                    ...
                    if (force > .5)
                        scatter(scene, { x: plant.root.x + bend * height * .62, y: plant.root.y }, force > 1 ? 3 : 1, '#ffdb63', true, height, true);
                    reaction.shed = 1.1;
                }
                else {
            ...
            rustle || (rustle = ['corn', 'sunflower', 'bush', 'hay', 'wheat', 'reeds', 'crop'].includes(type));
        }
        // At most the last few seconds of nearby scenery are active.
        if (scene.reactions.size > 100)
            scene.reactions.delete(scene.reactions.keys().next().value);
        return rustle;
    }
    function update(game, dt, previous) {
        ...
        scene.sound = Math.max(0, scene.sound - dt);
        scene.noise = Math.max(0, scene.noise - dt);
        scene.wet = Math.max(0, scene.wet - dt);
        scene.muddy = Math.max(0, scene.muddy - dt);
        for (const [id, r] of scene.reactions) {
            r.age += dt;
            r.shed = Math.max(0, r.shed - dt);
            if (r.age > (r.plant.prop.type === 'sunflower' ? 2.2 : 1.8))
                scene.reactions.delete(id);
        }
        ...
        scene.last = { ...p };
        scene.surface = surfaceAt(game, p);
        // No footsteps from teleports, menus, standing still, or pushing a solid wall.
        if (discontinuous || gap > Math.max(32, c.speed * dt * 1.6) || dt > .25) {
            scene.stride = 0;
            scene.wet = 0;
            scene.muddy = 0;
            return;
        }
        if (c.hidden || !c.moving || gap < .02) {
            scene.stride = 0;
            return;
        }
        if (SwimmingSystem.depthAt(game, from) <= .15 && SwimmingSystem.depthAt(game, p) > .15) {
            addMark(scene, p, 'ripple', 1.5);
            scatter(scene, p, 8, '#d0eee0');
            if (scene.sound <= 0) {
                AudioSystem.play('step-water', { volume: .42 });
                scene.sound = .28;
            }
        }
        ...
        for (let i = 1; i <= steps; i++) {
            ...
            if (surface === 'water') {
                ...
                scene.muddy = 3.5;
            if (scene.stride < (c.sneaking ? 21 : 17))
                continue;
            ...
            if (surface === 'water') {
                ...
                if (scene.muddy > 0)
                    addMark(scene, foot, 'mud', scene.muddy / 3.5, heading);
                else if (scene.wet > 0)
                    addMark(scene, foot, 'wet', scene.wet / 2.8, heading);
                if (c.sprinting && ['dirt', 'garden'].includes(surface))
                    scatter(scene, at, 2, '#cbb17c');
            }
            ...
            if (noisy && scene.sound <= 0) {
                ...
                AudioSystem.play(watery ? 'step-water' : muddy ? 'step-mud' : 'step-leaves', { volume: (c.sneaking ? .13 : c.sprinting ? .42 : .25) * (surface === 'garden' ? .7 : 1) * SkinSystem.power(c).noiseScale });
    ...
    function transform(context, prop, game) {
        if (!game)
            return;
    ...
    function drawGround(game) {
        ...
        for (const m of scene.marks) {
            ...
            ctx.globalAlpha = (1 - t) * (m.kind === 'ripple' ? .7 : .35) * m.size;
            if (m.kind === 'ripple') {
                // Rings stop at the bank instead of spilling onto the grass.
                ctx.beginPath();
                const pond = scene.layout.structures.pond;
                if (pond) {
                    ctx.ellipse(pond.x + pond.w / 2 - m.x, pond.y + pond.h / 2 - m.y, pond.w * .47, pond.h * .47, 0, 0, Math.PI * 2);
                }
                for (const h of scene.layout.habitats || []) {
                    ...
                    ctx.moveTo(p.x - m.x + rx, p.y - m.y);
                    ctx.ellipse(p.x - m.x, p.y - m.y, rx, ry, 0, 0, Math.PI * 2);
                }
                ctx.clip();
                const radius = 4 + t * (reduced() ? 8 : 23) * m.size;
                ctx.strokeStyle = '#daf2d9';
                ctx.lineWidth = 1.5;
                ctx.beginPath();
                ctx.ellipse(0, 0, radius, radius * .36, 0, 0, Math.PI * 2);
                ctx.stroke();
                if (t > .25) {
                    ctx.globalAlpha *= .5;
                    ctx.beginPath();
                    ctx.ellipse(0, 0, radius * .64, radius * .22, 0, 0, Math.PI * 2);
                    ctx.stroke();
                }
            }
            else {
                ctx.rotate(m.heading);
                ctx.strokeStyle = m.kind === 'mud' ? '#513f2c' : '#40665a';
                ctx.lineWidth = 2;
                ctx.beginPath();
                ctx.moveTo(-3, 0);
                ctx.lineTo(3, 0);
    ...
    function drawFeet(game) {
        const c = game.entities.chicken;
        if (c.hidden || SwimmingSystem.profile(game).depth > 0 || surfaceAt(game, feet(c)) !== 'water')
            return;
        const p = worldToScreen(feet(c));
        ctx.save();
        ctx.fillStyle = '#91c7be99';
        ctx.beginPath();
        ctx.ellipse(p.x, p.y, 13, 4, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.strokeStyle = '#d5e9cd';
        ctx.lineWidth = 1.5;
        ctx.beginPath();
        ctx.ellipse(p.x, p.y, 15, 5, 0, 0, Math.PI);
        ctx.stroke();
        ctx.restore();
    }
    function drawAir(game) {
        for (const q of scenes.get(game)?.particles || []) {
            const p = worldToScreen(q);
            ctx.save();
            ctx.translate(p.x, p.y - q.z);
            ctx.globalAlpha = Math.min(1, (q.life - q.age) * 3);
            ctx.fillStyle = q.color;
            if (q.petal) {
                ctx.rotate(reduced() ? 0 : q.age * 3);
                ctx.fillRect(-3, -1, 6, 3);
                ctx.fillRect(-1, -2, 3, 5);
                ctx.fillStyle = '#efbb36';
                ctx.fillRect(-2, 0, 4, 1);
            }
            else if (q.leaf) {
                ctx.rotate(reduced() ? 0 : q.age * 4);
                ctx.fillRect(-3, -1, 6, 3);
                ctx.fillStyle = '#657b3d';
    ...
    function hint(game) {
        const swim = SwimmingSystem.hint(game);
        if (swim)
            return swim;
        const s = surfaceAt(game, feet(game.entities.chicken));
        if (s === 'water')
            return 'Pé de galinha, modo pato. A água corta a trilha, mas correr faz um baita splosh!';
        if (s === 'mud')
            return 'Spa de barro! Passo mais lento e pegadas de brinde.';
        if (s === 'corn')
            return 'O milho é fofoqueiro: vá de mansinho para não chamar o lobo.';
        if (s === 'garden')
            return 'Devagar com a salada! De mansinho, as folhas fazem menos barulho.';
        if (s === 'sunflower')
            return 'Girassol bonito, companhia duvidosa… Se as flores sacudirem, saia para o lado!';
        return null;
    }
