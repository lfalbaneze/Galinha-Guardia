...
const WolfAI = (() => {
    ...
    function getConfig(game) {
        ...
        const config = Object.fromEntries(Object.keys(levels[0]).map(key => [key, levels[0][key] + (levels[3][key] - levels[0][key]) * progress]));
        ...
        for (const rect of nav.rects) {
            for (const x of [rect.x - 0.5, rect.x + rect.w + 0.5]) {
                for (const y of [rect.y - 0.5, rect.y + rect.h + 0.5]) {
                    const point = { x, y };
                    if (freePoint(point, nav))
                        nav.nodes.push(point);
                }
            }
        }
        return nav.nodes;
    ...
    function navigationEdges(nav) {
        ...
        for (let i = 0; i < nav.nodes.length; i += 1) {
            for (let j = i + 1; j < nav.nodes.length; j += 1) {
                if (!clearSegment(nav.nodes[i], nav.nodes[j], nav))
                    continue;
        ...
        if (clearSegment(start, end, nav)) {
            ...
            for (const index of [startIndex, endIndex]) {
                for (let j = 0; j < startIndex; j += 1) {
                    if (!clearSegment(nodes[index], nodes[j], nav))
                        continue;
            ...
            for (let step = 0; step < nodes.length; step += 1) {
                ...
                if (current < 0 || costs[current] === Infinity || current === endIndex)
                    break;
                visited.add(current);
                for (const edge of edges[current]) {
                    const next = costs[current] + edge.length;
                    if (next >= costs[edge.index])
                        continue;
            ...
            if (!Number.isFinite(costs[endIndex]))
                return [];
            path = [];
            for (let i = endIndex; i !== startIndex; i = previous[i])
                path.unshift(nodes[i]);
        }
        if (distance(center(wolf), start) > 0.01)
            path.unshift(start);
        return path.map(point => ({ x: point.x - offset.x, y: point.y - offset.y }));
    }
    function patrolPoints(config) {
        const points = [];
        ...
        for (const area of WORLD.areas) {
            ...
            if (config.level === 3)
                points.push({ x: area.x + area.w * 0.78, y: area.y + area.h * 0.78 });
        }
        if (config.level === 3)
    ...
    function makeSearch(wolf, config) {
    ...
    function beginSearch(wolf, config) {
        wolf.mode = "search";
    ...
    function turnToward(wolf, heading, dt, rate = 4.6) {
    ...
    function observe(wolf, point, game) {
        ...
        if (wolf.lastSight && wolf.sightAge > 0 && wolf.sightAge <= .25) {
            ...
            if (Math.hypot(vx, vy) <= maxSpeed * 1.1)
                velocity = { x: vx, y: vy };
        }
    ...
    function physicallyFree(point, nav) {
        return point.x >= nav.radius && point.y >= nav.radius && point.x <= WORLD.width - nav.radius && point.y <= WORLD.height - nav.radius &&
            nav.source.every(rect => rect.blocking === false || Math.hypot(point.x - clamp(point.x, rect.x, rect.x + rect.w), point.y - clamp(point.y, rect.y, rect.y + rect.h)) >= nav.radius - .001);
    }
    function moveToward(wolf, target, speed, dt) {
        ...
        if (distance(center(wolf), start) <= 3) {
            ...
            if (gap <= 32 && physicallyFree(from, nav)) {
                ...
                if ([.25, .5, .75, 1].every(t => physicallyFree({ x: from.x + (next.x - from.x) * t, y: from.y + (next.y - from.y) * t }, nav))) {
                    wolf.x = next.x - (wolf.hitbox?.ox || 0);
                    wolf.y = next.y - (wolf.hitbox?.oy || 0);
        ...
        if (!wolf.route.length)
            wolf.moveSpeed = 0;
        if (Math.abs(wolf.vx) > 0.1)
            wolf.facing = Math.sign(wolf.vx);
        wolf.anim = CharacterArt.advance(wolf.anim, 'wolf', distance(original, wolf), { speed: dt > 0 ? distance(original, wolf) / dt : 0 });
    ...
    function update(game, dt) {
        ...
        if (perception.visible) {
            ...
            if (wolf.mode === "chase" || perception.contact || wolf.awareness >= 1) {
                ...
                if (wolf.mode !== "alert")
                    wolf.alertReturnMode = wolf.mode;
                wolf.mode = "alert";
        ...
        if (!perception.visible && perception.heardPoint && wolf.hearingCooldown <= 0) {
            if (wolf.mode !== 'investigate')
                wolf.investigateReturnMode = wolf.mode === 'search' ? 'search' : 'patrol';
        ...
        if (wolf.mode === "search") {
            ...
            if (wolf.searchApproached)
                wolf.searchTime = Math.max(0, wolf.searchTime - dt);
        ...
        if (wolf.mode === "chase")
            target = pursuitTarget(wolf, game, config);
        if (wolf.mode === "patrol") {
        ...
        if (target) {
            ...
            if (arrived || (!wolf.route.length && wolf.routeTimer > 0)) {
                ...
                if (wolf.mode === "investigate")
                    wolf.scanTime = wolf.investigateTime;
                if (wolf.mode === "search") {
                    // An unreachable first waypoint also counts as inspected, avoiding an endless search.
                    if (wolf.searchIndex === 0)
                        wolf.searchApproached = true;
                    wolf.searchIndex += 1;
    ...
    function investigateSound(game, source, radius = 360, reportedPoint = source, acousticObstacles = OBSTACLES) {
        ...
        if (distance(wolf, source) > Math.min(360, radius) * Math.pow(.6, walls))
            return false;
        const config = getConfig(game);
        if (wolf.mode !== 'investigate')
            wolf.investigateReturnMode = wolf.mode === 'search' ? 'search' : 'patrol';
