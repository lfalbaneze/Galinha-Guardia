...
const GooseArt = (() => {
    ...
    function draw(context, goose, view) {
        ...
        if (typeof Sunlight !== 'undefined')
            Sunlight.cast(context, tile || image, Math.round(x - w / 2), Math.round(y + 14 - h + peck), w, Math.round(h - peck), y + 14, tile ? undefined : [crop.x, crop.y, crop.w, crop.h]);
        if (tile)
            context.drawImage(tile, Math.round(x - w / 2), Math.round(y + 14 - h + peck), w, Math.round(h - peck));
        else
            context.drawImage(image, crop.x, crop.y, crop.w, crop.h, Math.round(x - w / 2), Math.round(y + 14 - h + peck), w, Math.round(h - peck));
        if (goose.mode === 'stunned') {
            ...
            for (let i = 0; i < 3; i++) {
                ...
                context.fillRect(sx - 1, sy - 3, 3, 7);
            }
        }
        else if (goose.mode === 'notice') {
            context.font = 'bold 15px Trebuchet MS,sans-serif';
            context.textAlign = 'center';
            context.fillStyle = '#f8e6ac';
            context.fillText('?', x, y - 62);
        }
        context.restore();
        return true;
    }
