...
const GooseArt = (() => {
    function draw(context, goose, view) {
        ...
        c.save();
        c.translate(x, y);
        c.fillStyle = '#23352944';
        c.beginPath();
        c.ellipse(0, 13, 22, 7, 0, 0, Math.PI * 2);
        c.fill();
        c.scale(2, 2);
        c.imageSmoothingEnabled = false;
        const paint = (color, boxes) => {
            c.fillStyle = color;
            for (const [rx, ry, w, h] of boxes)
                c.fillRect(Math.round(rx), Math.round(ry), w, h);
        };
        ...
        if (goose.direction === 'left' || goose.direction === 'right') {
            ...
            if (flap)
                paint(outline, [[12 + reach, -25 + drop, 4, 1]]);
        }
        else {
            ...
            if (goose.direction === 'down') {
                ...
                if (flap)
                    paint(outline, [[-4, -26, 3, 1], [1, -26, 3, 1]]);
            }
            else
                paint(light, [[-4, -26, 2, 7], [-2, -18, 1, 12]]);
        }
        c.restore();
    }
    return { draw };
