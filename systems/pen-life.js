...
const PenLife = (() => {
    ...
    function inside(body, padding = 0) {
    ...
    function residents(game) {
        const list = (game.entities.animals || []).filter(a => a.rescued && inside(a));
        if (game.entities.goose?.rescued && inside(game.entities.goose))
            list.push(game.entities.goose);
        return list;
    }
    function canSee(game, body) {
    ...
    function resetGoose(goose) { wanderers.delete(goose); }
    function clearLeg(goose, to) {
        const from = getHitbox(goose), r = goose.hitbox.r + 1;
        const end = { x: to.x + goose.hitbox.ox, y: to.y + goose.hitbox.oy };
        return OBSTACLES.every(o => o.blocking === false || !DetectionSystem.segmentIntersectsRect(from, end, { x: o.x - r, y: o.y - r, w: o.w + r * 2, h: o.h + r * 2 }));
    }
    function newTarget(game, goose, w) {
        ...
        for (let i = 0; i < 24; i++) {
            ...
            if (distance(goose, p) < 30 || !clearLeg(goose, p))
                continue;
            const foot = { x: p.x + goose.hitbox.ox, y: p.y + goose.hitbox.oy };
            if (residents(game).some(a => a !== goose && distance(foot, getHitbox(a)) < goose.hitbox.r + a.hitbox.r + 2))
                continue;
            return p;
        }
        return null;
    }
    function updateGoose(game, goose, dt) {
        if (paused(game) || !goose.rescued || !Number.isFinite(dt) || dt <= 0)
            return;
        dt = Math.min(dt, .1);
        ...
        if (!inside(goose, goose.hitbox.r + 2)) {
            w.target = null;
            return;
        }
        if (w.pause > 0) {
        ...
        if (!w.target)
            w.target = newTarget(game, goose, w);
        if (!w.target) {
            w.pause = .4;
            return;
        }
        ...
        goose.moving = moved > .01;
        goose.state = goose.moving ? 'walk' : 'idle';
        if (goose.moving)
            Player.face(goose, goose.vx, goose.vy);
        ...
        if (blocked || distance(goose, target) < 2) {
            w.target = null;
            w.pause = blocked ? .25 : .45 + random(w) * .8;
            goose.activity = random(w) > .5 ? 'preen' : 'forage';
        }
    }
    function update(game, dt) {
        if (paused(game) || !Number.isFinite(dt) || dt <= 0)
            return;
        dt = Math.min(dt, .1);
        ...
        if (visit.comment) {
            ...
            if (visit.comment.remaining <= 0 || !canSee(game, visit.comment.speaker))
                visit.comment = null;
        }
        ...
        if (!visit.pending || visit.wait > 0 || visit.cooldown > 0 || visit.comment || chicken.hidden)
            return;
        ...
        if ((game.animalSpeechCooldown || 0) > 0 || (game.rescueNotice?.time || 0) > 0 ||
            ((game.entities.wolf.speechTime || 0) > 0 && canSee(game, game.entities.wolf)))
            return;
        ...
        visit.cooldown = 11;
        visit.serial++;
        visit.lastSpeaker = speaker.id;
        visit.lastLine = text;
    }
    function current(game) { return visits.get(game)?.comment || null; }
    function draw(game) {
        const comment = current(game);
        if (paused(game) || !comment || !inside(game.entities.chicken, 4) || !canSee(game, comment.speaker))
            return;
        const p = worldToScreen(comment.speaker);
        if (p.x < 0 || p.x > canvas.width || p.y < 112 || p.y > canvas.height)
            return;
        ctx.save();
        ...
        for (const word of comment.text.split(' ')) {
            ...
            if (line && ctx.measureText(next).width > width - 24) {
                lines.push(line);
                line = word;
            }
            else
        ...
        ctx.fill();
        ctx.beginPath();
        ctx.moveTo(tip - 5, y + height - 1);
        ctx.lineTo(tip + 5, y + height - 1);
        ctx.lineTo(tip, y + height + 6);
        ctx.closePath();
        ctx.fill();
        ctx.fillStyle = '#663d28';
