...
const FoxArt = (() => {
    ...
    function load(loader) {
        if (pending)
            return pending;
        pending = Promise.all([sheet.load(loader), amandaSheet.load(loader)]).then(result => { pending = null; return result.every(Boolean); });
        return pending;
    }
    ...
    return { draw, frameFor, frames, source, columns,
        load,
        install: (loader) => { sheet.install(loader); amandaSheet.install(loader); },
