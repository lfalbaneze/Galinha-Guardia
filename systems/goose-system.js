...
const GooseSystem = (() => {
    ...
    function clearLeg(from, to, margin = RADIUS + 1) {
        ...
        if (b.x < margin || b.y < margin || b.x > WORLD.width - margin || b.y > WORLD.height - margin)
            return false;
        return OBSTACLES.every(rect => rect.blocking === false || !DetectionSystem.segmentIntersectsRect(a, b, { x: rect.x - margin, y: rect.y - margin, w: rect.w + margin * 2, h: rect.h + margin * 2 }));
    }
    function chooseHome() {
        ...
        for (const gap of [65, 100, 140, 185]) {
            ...
            for (let i = 0; i < 4; i++)
                candidates.push(sides[(i + offset) % 4]);
        }
        for (const path of WORLD.paths)
            candidates.push({ x: path.x + path.w / 2, y: path.y + path.h / 2 });
    ...
    function initialize(game) {
        ...
        if (game.entities.goose.rescued)
    ...
    function rescue(game) {
        ...
        if (game.phase !== 'playing' || game.timeRemaining === 0 || !goose || !game.lake?.completed || game.lake.misses !== 3 || game.lake.gooseRescued)
            return false;
    ...
    function canNotice(game, goose, config) {
        const chicken = game.entities.chicken;
    ...
    function canCommit(game, goose, config) {
        const chicken = game.entities.chicken;
    ...
    function canApproach(game, goose, config) {
        const chicken = game.entities.chicken;
        return !chicken.hidden && chicken.invulnerable <= 0 && goose.grace <= 0 &&
            distance(chicken, goose.home) <= config.territory &&
            DetectionSystem.hasLineOfSight(getHitbox(goose), getHitbox(chicken));
    }
    function circle(goose, observed, config) {
        ...
        for (const sign of [side, -side]) {
            ...
            if (distance(target, goose.home) > config.territory - 20 || !clearLeg(goose, target))
                continue;
            goose.mode = 'circle';
            goose.tactic = 'flank';
            goose.target = target;
    ...
    function honk(game, goose) {
        if (goose.honkCooldown > 0)
            return;
    ...
    function recover(goose, config) {
        goose.mode = 'recover';
        goose.timer = .65;
        goose.cooldown = config.cooldown;
        goose.moving = false;
        goose.vx = 0;
        goose.vy = 0;
    }
    function peck(game, goose, config) {
        const chicken = game.entities.chicken;
        ...
        if (game.lake?.active)
            LakeChallenge.recordHit(game);
        else
            setStatus('PANTO fiscalizou suas penas de perto! Foi só um empurrão. Contorne o lago ou desvie da investida.');
        GameManager.save(game);
        return true;
    }
    function advance(game, goose, target, amount, config, attacking = false) {
        ...
        for (let i = 0; i < steps; i++) {
            ...
            if ((distance(next, goose.home) > config.territory + .01 && distance(next, goose.home) >= distance(goose, goose.home)) || !clearLeg(goose, next))
                return false;
            goose.x = next.x;
            goose.y = next.y;
            if (attacking && peck(game, goose, config))
                return false;
        }
        Player.face(goose, dx, dy);
        return length <= amount + .01;
    }
    function chargeTarget(goose, origin, observed, config) {
        ...
        for (let step = 6; step <= config.chargeSpeed * config.chargeSeconds; step += 6) {
            ...
            if (distance(next, goose.home) > config.territory || !clearLeg(target, next))
                break;
            target = next;
        }
        return target;
    }
    function makeRoom(goose, observed, config) {
        const candidates = [];
    ...
    function warn(game, goose, config) {
        ...
        if (distance(goose, target) < 36 && !circleVsCircle(goose, chicken)) {
            ...
            if (!makeRoom(goose, point(chicken), config))
                recover(goose, config);
            return;
        }
        ...
        if (round === 2 && feint)
            goose.challengeFeinted = true;
        if (round === 1 && !followup)
            goose.comboRemaining = 1;
        goose.comboFollowup = false;
        goose.earlyDodge = false;
        goose.tactic = feint ? 'bluff' : round === 2 ? 'rush' : round === 1 ? 'double' : goose.tactic === 'flank' ? 'flank' : 'direct';
        goose.mode = feint ? 'feint' : 'warning';
        goose.timer = feint ? .6 : followup ? Math.max(.72, config.warning * .78) : config.warning;
    ...
    function patrolTarget(goose) {
        ...
        if (goose.patrolIndex % 2 === 0)
            return point(goose.home);
    ...
    function rememberReturn(goose, before) {
        if (clearLeg(goose, goose.home)) {
            goose.returnPath = [];
            return;
        }
        const route = goose.returnPath || (goose.returnPath = []);
        const shortcut = route.findIndex(p => clearLeg(goose, p));
        if (shortcut >= 0)
            route.splice(shortcut + 1);
        else
            route.push(before);
    }
    function goHome(game, goose, config, dt) {
        const route = goose.returnPath || (goose.returnPath = clearLeg(goose, goose.home) ? [] : [point(goose.anchor)]);
        const destination = route.length ? route[route.length - 1] : goose.home;
        if (advance(game, goose, destination, game.lake?.completed ? 55 * dt : 115 * dt, config) && route.length)
            route.pop();
    ...
    function update(game, dt) {
        ...
        if (canApproach(game, goose, config)) {
            ...
            goose.lastObserved = point(chicken);
            goose.observationAge = 0;
            if (goose.mode === 'warning' && goose.timer > config.warning * .5) {
                ...
                if (Math.abs(dx * (chicken.y - goose.anchor.y) - dy * (chicken.x - goose.anchor.x)) / len > 48)
                    goose.earlyDodge = true;
            }
        }
        ...
        if (goose.mode === 'patrol') {
            ...
            if (!canApproach(game, goose, config) || goose.timer <= 0)
                recover(goose, config);
            else if (canNotice(game, goose, config)) {
                goose.mode = 'notice';
                goose.timer = .45;
            }
            else {
                ...
                if (distance(before, goose) < .01 && !makeRoom(goose, point(chicken), config))
                    recover(goose, config);
            }
        }
            ...
            if (goose.comboFollowup && game.lake?.active) {
                ...
                if (goose.timer <= 0 && !(flanking && circle(goose, goose.noticedPoint, config)))
                    warn(game, goose, config);
            }
        }
            ...
            if (!canApproach(game, goose, config))
                recover(goose, config);
            else if (advance(game, goose, goose.target, 75 * dt, config) || goose.timer <= 0 || distance(before, goose) < .01)
                warn(game, goose, config);
        }
        else if (goose.mode === 'reposition') {
            if (game.entities.chicken.hidden || distance(game.entities.chicken, goose.home) > config.territory)
                recover(goose, config);
            else {
                const arrived = advance(game, goose, goose.target, 70 * dt, config);
                if (arrived) {
                    goose.mode = canNotice(game, goose, config) ? 'notice' : 'approach';
                    goose.timer = goose.mode === 'notice' ? .35 : 2.5;
                }
                else if (goose.timer <= 0 || distance(before, goose) < .01)
                    recover(goose, config);
            }
        }
        else if (goose.mode === 'feint') {
            if (game.lake?.active && goose.timer <= 0 && canApproach(game, goose, config))
                warn(game, goose, config);
            ...
            if (!canCommit(game, goose, config))
                recover(goose, config);
            else if (goose.timer <= 0) {
                goose.mode = 'charge';
                goose.timer = config.chargeSeconds;
            }
        }
            ...
            if (!peck(game, goose, config)) {
                ...
                if (goose.mode === 'charge' && (arrived || goose.timer <= 0 || distance(before, goose) < .01)) {
                    if (canApproach(game, goose, config) && !goose.chargeHit) {
                        ...
                        if (Math.abs(cross) > 800)
                            goose.dodgeSide = Math.sign(cross);
                    }
                    ...
                    if (game.lake?.active && (goose.comboRemaining || 0) > 0 && !goose.chargeHit && distance(goose, goose.anchor) >= 36) {
                        goose.comboRemaining = Math.max(0, (goose.comboRemaining || 0) - 1);
                        goose.comboFollowup = true;
                        goose.mode = 'notice';
                        goose.timer = .25;
                        game.lake.feedback = 'combo';
                        game.lake.notice = 1.4;
                        ...
                        if (!game.lake?.completed) {
                            ...
                            if (counted || distance(goose, goose.anchor) >= 36) {
                                goose.mode = 'stunned';
                                goose.timer = counted ? (game.lake?.counterWindow || 3) : 1.05;
                            }
                        }
                    }
                }
            ...
            if (goHome(game, goose, config, dt))
            ...
            if (goose.cooldown <= 0 && canApproach(game, goose, config)) {
                goose.mode = 'approach';
                goose.timer = 2.5;
            }
                ...
                goose.patrolIndex = 1;
                goose.timer = .8;
                goose.cooldown = Math.max(goose.cooldown, .8);
                goose.anchor = point(goose.home);
                goose.tactic = 'direct';
            }
        }
        if (distance(before, goose) > .01)
            rememberReturn(goose, before);
        goose.vx = (goose.x - before.x) / dt;
    ...
    function restore(game, saved) {
        initialize(game);
        const goose = game.entities.goose;
        if (!goose || goose.rescued || !isPoint(saved))
            return;
        const record = saved;
        ...
        if (distance(saved, goose.home) > TERRITORY || distance(anchor, goose.home) > TERRITORY ||
            !clearLeg(saved, saved) || !clearLeg(goose.home, anchor) || !clearLeg(anchor, saved))
            return;
        goose.x = saved.x;
        goose.y = saved.y;
        goose.anchor = point(anchor);
    ...
    function drawTerritory(game) {
        ...
        if (goose.mode === 'stunned' && (game.lake?.counterWindow || 0) > 0) {
            ...
            ctx.setLineDash([]);
            ctx.fillStyle = '#7bd78c25';
            ctx.beginPath();
            ctx.ellipse(at.x, at.y + OFFSET_Y, 40, 23, 0, 0, Math.PI * 2);
            ctx.fill();
            ctx.lineWidth = 4;
            ctx.strokeStyle = '#daf6a0';
            ctx.beginPath();
            ctx.ellipse(at.x, at.y + OFFSET_Y, 40, 23, 0, -Math.PI / 2, -Math.PI / 2 + Math.PI * 2 * ratio);
            ctx.stroke();
        }
        if (goose.mode === 'warning' && visible(game, goose)) {
            ...
            ctx.setLineDash([]);
            ctx.lineCap = 'round';
            ctx.strokeStyle = '#f4bf5730';
            ctx.lineWidth = contact;
            ctx.beginPath();
            ctx.moveTo(from.x, from.y + OFFSET_Y);
            ctx.lineTo(to.x, to.y + OFFSET_Y);
            ctx.stroke();
            ctx.strokeStyle = goose.tactic === 'double' ? '#f69362' : goose.tactic === 'rush' ? '#f07862' : '#f4bf57';
            ctx.lineWidth = 4;
            ctx.setLineDash([8, 6]);
            ctx.beginPath();
            ctx.moveTo(from.x, from.y + OFFSET_Y);
            ...
            ctx.setLineDash([]);
            ctx.beginPath();
            ctx.arc(to.x, to.y + OFFSET_Y, 6, 0, Math.PI * 2);
            ctx.stroke();
        }
        ctx.restore();
    }
    function drawIndicator(game) {
        ...
        if (!speaking) {
            ...
            ctx.fillStyle = '#243f32ed';
            ctx.beginPath();
            ctx.roundRect(x - width / 2, y - 17, width, 29, 8);
            ctx.fill();
            ctx.fillStyle = '#fff1be';
            ctx.fillText(line, x, y + 2, width - 18);
            if (goose.mode === 'warning') {
                ctx.fillStyle = '#243f32';
                ctx.fillRect(x - width / 2 + 8, y + 16, width - 16, 5);
                ctx.fillStyle = '#f4d28c';
                ctx.fillRect(x - width / 2 + 9, y + 17, (width - 18) * clamp(1 - goose.timer / (goose.warningDuration || getConfig(game).warning), 0, 1), 3);
            }
        }
        ctx.restore();
    }
