...
const ScarecrowArt = (() => {
    ...
    function pose(c, index, x, y, scale, pivotX, pivotY, flip = false, elevation = 0) {
    ...
    function drawPost(c, s, view) {
        ...
        c.save();
        c.fillStyle = 'rgba(44,48,27,.2)';
        c.beginPath();
        c.ellipse(x + 3, y + 2, 28, 6, 0, 0, Math.PI * 2);
        c.fill();
        pose(c, 0, x, y, 108 / 472, 205, 472);
        c.restore();
    }
    function drawCrow(c, b, index, clock, view) {
        if (b.opacity <= 0)
            return;
