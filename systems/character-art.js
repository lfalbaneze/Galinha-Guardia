...
const CharacterArt = (() => {
  ...
  function eyes3Q(c, a, farX, nearX, y, big = false) {
    c.save(); c.translate(farX, y - .6); c.scale(.72, .85);
    eye(c, 0, 0, a.blink, true, a.hurt, a.mood, 1); c.restore();
    c.save(); c.translate(nearX, y); c.scale(big ? 1 : .82, big ? 1 : .88);
    eye(c, 0, 0, a.blink, true, a.hurt, a.mood, -1); c.restore();
  }
  ...
  function chickenHeadwear(c, a, axial, back) {
    ...
    if (a.skin === "punk") {
      ...
      if(!back) { if(axial) for(const sx of [-1,1]) eye(c,sx*7,-29,a.blink,false,false,a.mood,-sx);else eyes3Q(c,a,9,21,-31); }
  ...
  function chicken(c, a) {
    ...
    c.restore();
    eyes3Q(c,a,9,21,-31,true); cheek(c, 20, -22);
    if (a.hidden) {
  ...
  function duck(c, a) {
    ...
    eyes3Q(c,a,7,18,-26); cheek(c, 18, -18, "#efa377");
  }
  function chick(c, a, axial, back) {
    ...
    if(axial) {
      ...
      if(back) {
        shape(c,"#ffe9a6",p=>{p.moveTo(-4,3);p.lineTo(-5,-3);p.lineTo(0,0);p.lineTo(4,-3);p.lineTo(4,4);},1);
        if(a.lookBack) {
          oval(c,5,-16,11,10,"#f3cd67"); eyes3Q(c,a,1,10,-17);
          shape(c,"#da9a43",p=>{p.moveTo(12,-14);p.lineTo(19,-11);p.lineTo(12,-9);},1);
        }
        return;
      }
      ...
      eyes3Q(c,a,0,9,-15);cheek(c,8,-8,"#edba79");
  ...
  function sheep(c, a, lamb) {
    ...
    eyes3Q(c,a,15,24,-14); cheek(c, 22, -7, "#c48c79");
  ...
  function rabbit(c, a) {
    ...
    eyes3Q(c,a,7,18,-15); cheek(c, 16, -7);
  ...
  function pig(c, a) {
    ...
    eyes3Q(c,a,9,20,-18); cheek(c, 11, -9, "#dc8c91");
  ...
  function hoofed(c, a, species) {
    ...
    eyes3Q(c,a,10,21,-17); cheek(c, 13, -9);
  ...
  function pet(c, a, cat) {
    ...
    eyes3Q(c,a,10,21,-19); cheek(c, 13, -11);
  ...
  function verticalBird(c, a, species, back) {
    ...
    if (back) {
      line(c, hen ? "#d4bd94" : "#dcad4c", 1.4, p => { p.moveTo(-5, -17); p.quadraticCurveTo(0, -14, 5, -17); });
      if (a.lookBack && !hen) {
        oval(c,6,-27,13,12,coat); eyes3Q(c,a,1,12,-29);
        oval(c,17,-21,9,4,"#e9a547");
      }
      if(hen) chickenHeadwear(c,a,true,true); return;
    }
  ...
  function verticalAnimal(c, a, species, back) {
    ...
    const headBack = back && !a.lookBack;
    const headY = headBack ? -25 : -20;
    c.save(); c.translate(a.mood === "search" ? a.wave * 1.3 : 0, (a.mood === "sniff" ? 4 + a.wave : 0));
    if (back && a.lookBack) { c.translate(6,-3); c.rotate(.12); }
    for (const side of [-1, 1]) {
      if (rabbit || donkey) {
        const tilt = side * (.18 + a.wave * .028);
        oval(c, side * 8, headY - 17, rabbit ? 4.5 : 4, rabbit ? 14 : 13, coat, true, tilt);
        if (!headBack) oval(c, side * 8, headY - 18, 1.8, 8.5, "#dcb1a2", false, tilt);
        ...
        if (!headBack) shape(c, "#cfa496", p => { p.moveTo(side * 8, headY - 9); p.lineTo(side * 14, headY - 14); p.lineTo(side * 14, headY - 5); }, 0);
      } else {
  ...
  function draw(c, species, x, y, options = {}) {
    ...
    const a = { lookBack: !!options.lookBack, step, hidden: blend, mood, anim, skin, wave: moving ? wave : Math.sin(anim * .9), sprinting, hurt: mood === "hurt", blink: ((anim % 23) + 23) % 23 > 22.35 };
    ...
    const glance = direction === "up" && a.lookBack;
    c.save();
    if (glance && !["chicken", "duck", "chick"].includes(species)) { c.translate(6,-3); c.rotate(.12); }
    else if (glance && species !== "chicken") c.translate(6,-3);
    emotion(c,a,species,axial,direction === "up" && (!glance || species === "chicken"));
    c.restore();
    c.restore();
  }
