...
const MapManager = {
    ...
    update(game, dt) {
        ...
        if (areaTextEl.textContent !== region.name)
    ...
    drawTransition(game) {
        ...
        if (!t || t.time <= 0 || game.phase !== "playing")
            return;
        ctx.save();
        ctx.globalAlpha = Math.min(1, t.time);
        ctx.fillStyle = "rgba(249, 245, 223, .94)";
        ctx.fillRect(290, 22, 310, 46);
        ctx.fillStyle = "#3c5a36";
        ctx.textAlign = "center";
        ctx.font = "bold 19px sans-serif";
        ctx.fillText(t.name, 445, 52);
        ctx.restore();
    },
};
