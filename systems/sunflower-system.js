...
const SunflowerSystem = (() => {
    ...
    const ambushes = new WeakMap();
    const overlap = (a, b, gap = 0) => a.x < b.x + b.w + gap && a.x + a.w + gap > b.x && a.y < b.y + b.h + gap && a.y + a.h + gap > b.y;
    const gap = (p, r) => Math.hypot(p.x - clamp(p.x, r.x, r.x + r.w), p.y - clamp(p.y, r.y, r.y + r.h));
    function field(layout = WORLD.layout) {
        const cached = fields.get(layout);
        if (cached)
            return cached;
        const s = layout.structures, occupied = [s.pond, ...(layout.plots || []), ...(layout.clearings || [])];
        for (const [items, type] of [[s.coops, 'coop'], [s.silos, 'silo'], [s.hayBales, 'hay'], [s.stables || [], 'stable'],
            [s.troughs || [], 'trough'], [s.paddockFences || [], 'paddock-fence'], [[s.barn], 'barn']])
            for (const p of items)
                occupied.push(FarmDetails.shape({ ...p, type }));
        for (const p of layout.vegetation)
            occupied.push(FarmDetails.shape(p));
        for (const h of layout.habitats || [])
            occupied.push({ x: h.x - 125, y: h.y - 94, w: 250, h: 188 });
        ...
        for (const [w, h] of [[276, 228], [240, 200], [204, 176]]) {
            let best = Infinity;
            for (let y = 96; y + h < WORLD.height - 60; y += 40)
                for (let x = 60; x + w < WORLD.width - 60; x += 40) {
                    const r = { x, y, w, h }, center = { x: x + w / 2, y: y + h * .64 };
                    if (distance(center, layout.start) < 510 || occupied.some(p => overlap(r, p, 18)) ||
                        tracks.some(p => overlap(r, p, 20)) || anchors.some(p => gap(p, r) < 76))
                        continue;
                    ...
                    if (score < best) {
                        best = score;
                        plot = { ...r, id: 'sunflower-field', kind: 'sunflower', areaId: area?.id || 'granja' };
                    }
                }
            if (plot)
                break;
        }
        const props = [], bed = plot;
        if (bed) {
        ...
        const result = { plot: bed, props };
        fields.set(layout, result);
        return result;
    }
    const plot = (layout = WORLD.layout) => field(layout).plot;
    function contains(p) {
        const r = plot();
        return !!r && p.x > r.x + 12 && p.x < r.x + r.w - 12 && p.y > r.y + 58 && p.y < r.y + r.h - 20;
    }
    function reset(game, cooldown = 7) {
        ambushes.set(game, { layout: WORLD.layout, mode: 'none', cooldown, timer: 0, elapsed: 0, target: { x: 0, y: 0 }, origin: { x: 0, y: 0 }, heading: 0 });
    }
    function data(game) {
        if (ambushes.get(game)?.layout !== WORLD.layout)
            reset(game);
    ...
    const concealed = (game) => ['hidden', 'warning'].includes(mode(game));
    const canCatch = (game) => ['none', 'approach', 'dash'].includes(mode(game));
    function cancel(game) {
        ...
        a.mode = 'none';
        a.cooldown = 16;
        game.entities.wolf.route = [];
        game.entities.wolf.routeTimer = 0;
    }
    function stop(wolf, dt) {
        wolf.vx = 0;
        wolf.vy = 0;
        wolf.moveSpeed = 0;
        wolf.moving = false;
    ...
    function updateWolf(game, dt, config) {
        ...
        if (wolf.mode === 'frightened' || wolf.mode === 'inspect' || wolf.pauseTimer > 0 || wolf.huntUnlockTimer > 0) {
            cancel(game);
            return false;
        }
        const bed = plot();
        ...
        if (a.mode === 'none') {
            if (a.cooldown > 0 || wolf.mode !== 'patrol' || distance(wolf, hide) > 900 || DetectionSystem.canSee(wolf, chicken, config))
                return false;
            if (!WolfAI.findPath(wolf, hide).length) {
                a.cooldown = 4;
                return false;
            }
            a.mode = 'approach';
        ...
        a.elapsed += dt;
        if (a.mode === 'approach') {
            if (DetectionSystem.canSee(wolf, chicken, config) || a.timer === 0) {
                cancel(game);
                return false;
            }
            if (WolfAI.moveTo(wolf, a.target, config.patrolSpeed, dt)) {
                ...
                a.heading = Math.atan2(edge.y - wolf.y, edge.x - wolf.x);
                wolf.heading = a.heading;
                wolf.speechTime = 0;
            }
            return true;
        }
        if (a.mode === 'hidden') {
            stop(wolf, dt);
            wolf.heading = a.heading + Math.sin(a.elapsed * .7) * .8;
            if (chicken.invulnerable <= 0 && DetectionSystem.canSee(wolf, chicken, { ...config, range: 235, fov: Math.PI * 1.2 })) {
                ...
                a.origin = { x: wolf.x, y: wolf.y };
                a.target = { x: wolf.x + dx / length * Math.min(255, length + 42), y: wolf.y + dy / length * Math.min(255, length + 42) };
                a.mode = 'warning';
                a.timer = game.difficultyKey === 'easy' ? 1.4 : ['hard', 'hardcore'].includes(game.difficultyKey) ? .95 : 1.15;
                wolf.lastKnown = { x: chicken.x, y: chicken.y };
                wolf.heading = Math.atan2(dy, dx);
                Player.face(wolf, dx, dy);
                AudioSystem.play('fox-rustle', { volume: .6 });
                if (WildlifeRules.onScreen(wolf, 90))
                    setStatus('Tem focinho nos girassóis! Saia para o lado da faixa antes do bote!');
            }
            else if (a.timer === 0)
                cancel(game);
            return true;
        ...
        if (a.mode === 'dash') {
            ...
            wolf.vx = (wolf.x - before.x) / dt;
            wolf.vy = (wolf.y - before.y) / dt;
            wolf.moving = distance(before, wolf) > .01;
            wolf.anim += dt * 14;
            if (result !== 'moving' || a.timer === 0) {
                a.mode = 'recover';
                a.timer = 1.2;
                stop(wolf, 0);
            }
    ...
    function drawPlant(c, p, game) {
        ...
        c.strokeStyle = '#382919';
        c.lineWidth = 4;
        c.stroke();
        c.strokeStyle = '#8caf40';
        c.lineWidth = 2;
        c.stroke();
        for (const [y, s] of [[-18, -1], [-33, 1], [-46, -1]]) {
            c.beginPath();
            c.moveTo(0, y + 5);
            c.quadraticCurveTo(s * 5, y - 11, s * 15, y - 9);
            c.quadraticCurveTo(s * 16, y + 1, 0, y + 5);
            c.fillStyle = s < 0 ? '#78a237' : '#a0bf43';
            c.fill();
            c.strokeStyle = '#382919';
            c.lineWidth = 1.5;
            c.stroke();
        ...
        for (let i = 0; i < 10; i++) {
            ...
            c.rotate(angle);
            c.beginPath();
            c.ellipse(0, -11.5, 4.5, 6, 0, 0, Math.PI * 2);
            c.fillStyle = i < 5 ? '#ffd65b' : '#edb538';
            c.fill();
            c.strokeStyle = '#6b421c';
            c.lineWidth = 1.5;
            c.stroke();
            c.beginPath();
            c.moveTo(0, -15);
            c.lineTo(0, -12);
            c.strokeStyle = '#ffed9c';
            c.lineWidth = 1.5;
            c.stroke();
            c.restore();
        }
        ...
        c.stroke();
        c.beginPath();
        c.ellipse(-2, -2.5, 4.5, 3, 0, 0, Math.PI * 2);
        c.fillStyle = '#ba7b3a';
        c.fill();
    ...
    function drawWarning(game) {
        const a = ambushes.get(game);
        if (mode(game) !== 'warning' || !a)
            return;
        const from = worldToScreen(a.origin), to = worldToScreen(a.target);
        ctx.save();
        ctx.strokeStyle = '#d17a4c55';
        ctx.lineWidth = 2 * (game.entities.wolf.hitbox.r + game.entities.chicken.hitbox.r);
        ctx.beginPath();
        ...
        ctx.fillRect(from.x - 9, from.y - 92, 18, 29);
        ctx.fillStyle = '#ffe593';
        ctx.fillRect(from.x - 3, from.y - 87, 6, 13);
        ctx.fillRect(from.x - 3, from.y - 70, 6, 4);
        // Two eyes are visible through the moving flowers; the body stays concealed until the leap.
        ctx.fillStyle = '#ffe78b';
        ctx.fillRect(from.x - 9, from.y - 35, 5, 3);
        ctx.fillRect(from.x + 4, from.y - 35, 5, 3);
        ctx.restore();
    }
    return { plot, props: (layout) => field(layout).props, contains, reset, cancel, mode, concealed, canCatch, updateWolf, drawGround, drawPlant, drawWarning };
})();
