...
const HidingSpots = (() => {
    ...
    function hasBonusClue(chicken, chick) {
        ...
        if (!spot || distance(chicken, chick) >= 280)
            return false;
        if (candidate(chicken)?.id === spot.id)
            return true;
    ...
    function pill(x, y, width, title, detail, hidden, exposed = false, progress = 0) {
        ...
        if (progress > 0) {
            ctx.fillStyle = '#f1cb68';
            ctx.fillRect(-width / 2 + 4, 27, (width - 8) * Math.min(1, progress), 3);
        }
        ctx.restore();
    }
    function drawIndicators(game) {
        ...
        if (chicken.hidden) {
            if (!celebrating || exposed)
                pill(p.x, p.y - 73, bonus ? 220 : 185, exposed ? "Ele viu você! Saia daí!" : bonus ? 'Segure C · procurar pintinho' : "Escondida · E para sair", '', true, exposed, bonus && !exposed ? bonus.discoveryTime / .85 : 0);
        }
        else if (spot) {
