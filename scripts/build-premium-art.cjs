...
const root=path.resolve(__dirname,'..'),folder='assets/sprites/cartoon-104';
...
async function pack(item){
 ...
 const occupied=(x,y)=>rgba[(y*image.width+x)*4+3]>=160;
 ...
 for(let row=0;row<rows.length;row++){
  ...
  for(const [x0,x1] of cols){let l=x1,t=y1,r=x0,b=y0,pixels=0;
   ...
   for(let y=t;y<b;y++)for(let x=l;x<r;x++)if(mask[(y-y0)*cw+x-x0]){
    ...
    for(let k=0;k<4;k++)cutPixels.data[dest+k]=rgba[src+k];
   }
   cutContext.putImageData(cutPixels,0,0);
   frames.push({x:l,y:t,w:r-l,h:b-t,cx:mass?moment/mass:(r-l)/2,cut});
  }
 }
 ...
 const modelHeight=Math.min(maxH,...bounds.map(b=>maxW*b.h/b.w)),scales=bounds.map(b=>modelHeight/b.h);
 const cellW=Math.ceil(Math.max(...frames.map((f,i)=>f.w*scales[Math.floor(i/count)])))+6,cellH=Math.ceil(modelHeight)+6;
 const out=createCanvas(cellW*count,cellH*8),ctx=out.getContext('2d');ctx.imageSmoothingEnabled=false;
 const runtime=`${folder}/runtime/${item.name}.png`,poses={};
 for(const [row,direction]of ['right','downright','down','upright','up'].entries()){
  ...
  const packed=frames.slice(row*count,(row+1)*count).map((f,i)=>{
   const w=Math.round(f.w*scale),h=Math.round(f.h*scale),x=i*cellW+3,y=row*cellH+3;
   ctx.drawImage(style.tile(f.cut,[0,0,f.w,f.h],w,h),x,y);
   return {src:runtime,x,y,w,h,cx:Math.round(f.cx*scale),bottom:h,top:0,width:w};
  });
  poses[direction]={frames:packed,idleIndex:0,cx:packed[0].cx,bottom:Math.max(...packed.map(f=>f.h)),top:0,width:Math.max(...packed.map(f=>f.w)),flip:false};
 }
 for(const [row,[direction,original]]of [['left','right'],['downleft','downright'],['upleft','upright']].entries()){
  poses[direction]={...poses[original],frames:poses[original].frames.map(f=>{
   const y=cellH*(row+5)+3;ctx.save();ctx.translate(f.x+f.w,y);ctx.scale(-1,1);ctx.drawImage(out,f.x,f.y,f.w,f.h,0,0,f.w,f.h);ctx.restore();return {...f,y,cx:f.w-f.cx};
  }),flip:false};
 }
 fs.writeFileSync(path.join(root,runtime),out.toBuffer('image/png'));
 return {scale:1,cycle:4,edition:104,poses,source:runtime,width:out.width,height:out.height};
}
(async()=>{
 fs.mkdirSync(path.join(root,folder,'runtime'),{recursive:true});
 const manifest=JSON.parse(fs.readFileSync(path.join(root,folder,'manifest.json'),'utf8')),data={};
 const required=Object.keys(sizes),names=new Set(manifest.map(i=>i.name));
 if(names.size!==required.length||required.some(n=>!names.has(n)))throw Error('Refusing incomplete cast export; missing: '+required.filter(n=>!names.has(n)).join(', '));
 for(const item of manifest)if(item.rows!==5||item.columns!==8||!fs.existsSync(path.join(root,folder,'raw',item.name+'.png')))throw Error('Invalid or missing source: '+item.name);
 for(const item of manifest)data[item.name]=await pack(item);
 const wildlife={};for(const name of ['fox','amanda','thor','goose','goose-alert','owl','owl-flight','scarecrow','crow'])if(data[name]){
  const d=data[name];wildlife[name]={src:d.source,width:d.width,height:d.height,columns:d.poses.right.frames.length,frames:['down','right','up','left','downright','upright','downleft','upleft'].flatMap(dir=>d.poses[dir].frames.map(({x,y,w,h,cx})=>({x,y,w,h,cx})))};
 }
 delete data['goose-alert'];
 delete data['owl-flight'];
 fs.writeFileSync(path.join(root,'systems/premium-art-data.js'),'/* Generated by scripts/build-premium-art.cjs. */\nconst PremiumArtData = '+JSON.stringify(data,null,2)+';\nObject.assign(SpriteData, PremiumArtData);\nconst PremiumWildlifeData = '+JSON.stringify(wildlife,null,2)+';\n');
 console.log(`Exported ${Object.keys(data).length} characters, Panto's threat display and owl flight, all with eight directions.`);
