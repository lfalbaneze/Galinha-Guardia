...
const ThorSystem = (() => {
  ...
  type Bone=Farm.Point & {id:string};
  const boneCache=new WeakMap<Farm.GameState,{layout:Farm.Layout;cycle:number;cost:number;batch:number;spots:Bone[]}>();
  const cost=(game:Farm.GameState):number=>game.difficultyKey==='easy'?0:game.difficultyKey==='normal'?2:
    game.difficultyKey==='hard'?4:5+(game.thorVisit?.visits||0);
  const boneCount=(game:Farm.GameState):number=>game.thorVisit?.boneIds.length||0;
  const active=(game:Farm.GameState):boolean=>!!game.thorRescue;
  const available=(game:Farm.GameState):boolean=>game.phase==='playing'&&!active(game)&&
    game.lives>0&&game.lives<MAX_LIVES&&(!cost(game)||(!game.lake?.active&&!SwimmingSystem.profile(game).swimming));
  const canCall=(game:Farm.GameState):boolean=>available(game)&&cost(game)>0&&boneCount(game)>=cost(game);
  ...
  function initialize(game: Farm.GameState): void {
    ...
    game.entities.thor=null;game.thorRescue=undefined;game.thorNotice=undefined;game.thorBoneNotice=undefined;boneCache.delete(game);
  }
  function boneSpots(game:Farm.GameState):Bone[] {
    ...
    if(needed<=0)return [];
    if(cached?.layout===layout&&cached.cycle===cycle&&cached.cost===cost(game)&&cached.batch===batch)return cached.spots;
    const player=game.entities.chicken,occupied=FarmArt.getProps(layout).map(FarmDetails.shape);
    const pond=layout.structures.pond,candidates:Farm.Point[]=[],seen=new Set<string>();
    for(const road of [...layout.paths,...layout.lanes||[]]) {
      ...
      for(let at=32;at<length-24;at+=72) {
        ...
        if(seen.has(key)||distance(p,layout.start)<190||p.x<50||p.y<50||p.x>WORLD.width-50||p.y>WORLD.height-50)continue;
        seen.add(key);
        if(!WildlifeRules.clear(p,p,player.hitbox)||EnvironmentSystem.surfaceAt(game,feet)==='water'||
          ...
          occupied.some(r=>p.x>r.x-20&&p.x<r.x+r.w+20&&p.y>r.y-24&&p.y<r.y+r.h+20))continue;
        candidates.push(p);
      }
    }
    const hash=(p:Farm.Point):number=>{
      ...
      n=Math.imul(n^(n>>>16),0x45d9f3b);return ((n^(n>>>16))>>>0)/4294967296;
    };
    const spots:Bone[]=[],body={...layout.start,radius:player.radius,hitbox:player.hitbox};
    for(let i=0;i<needed;i++) {
      const score=(p:Farm.Point):number=>spots.length?Math.min(...spots.map(s=>distance(p,s)))+hash(p)*180:
        800-Math.abs(distance(p,layout.start)-650)+hash(p)*300;
      candidates.sort((a,b)=>score(b)-score(a));
      const index=candidates.findIndex(p=>{
        if(spots.some(s=>distance(p,s)<240))return false;
    ...
    boneCache.set(game,{layout,cycle,cost:cost(game),batch,spots});return spots;
  }
  function bones(game:Farm.GameState):Bone[] {
    if(!cost(game))return [];
    return boneSpots(game).filter(b=>!game.thorVisit?.boneIds.includes(b.id));
  }
  function collectBones(game:Farm.GameState):void {
    if(!cost(game)||boneCount(game)>=cost(game)||game.entities.chicken.hidden)return;
    for(const bone of bones(game)) {
      if(distance(game.entities.chicken,bone)>30||!DetectionSystem.hasLineOfSight(getHitbox(game.entities.chicken),bone))continue;
      game.thorVisit!.boneIds.push(bone.id);
      const count=boneCount(game);game.thorBoneNotice={time:3,count};
      AudioSystem.play('pop',{volume:.45});spawnBurst(bone.x,bone.y,'#ffe7a9',8);
      ...
      if(count>=cost(game))break;
    }
  }
  function drawBones(game:Farm.GameState):void {
    for(const bone of bones(game)) {
      if(!WildlifeRules.onScreen(bone,40))continue;
      ...
      ctx.fillStyle='#38533845';ctx.beginPath();ctx.ellipse(0,9,14,4,0,0,Math.PI*2);ctx.fill();
      ctx.translate(0,bob);
      ctx.strokeStyle='#f6d67c90';ctx.lineWidth=1.5;ctx.beginPath();ctx.ellipse(0,3,20,13,0,0,Math.PI*2);ctx.stroke();
      const outline=[[-14,-5],[-10,-8],[-6,-6],[-5,-3],[5,-3],[6,-6],[10,-8],[14,-5],[14,-1],[12,1],[14,4],[13,8],[9,9],[6,7],[5,4],[-5,4],[-6,7],[-9,9],[-13,8],[-14,4],[-12,1],[-14,-1]];
      ctx.beginPath();ctx.moveTo(outline[0][0],outline[0][1]);for(const [x,y]of outline.slice(1))ctx.lineTo(x,y);ctx.closePath();
      ctx.fillStyle='#f8ebc5';ctx.fill();ctx.strokeStyle='#736044';ctx.lineWidth=2;ctx.stroke();
      ctx.fillStyle='#fff9e6';ctx.fillRect(-10,-5,3,3);ctx.fillRect(-4,-2,10,2);ctx.fillRect(8,-5,3,3);
      ctx.restore();
    }
  }
  ...
  function startRescue(game: Farm.GameState): boolean {
    if(!available(game))return false;
    const visit=game.thorVisit!;
    if(cost(game)) { if(boneCount(game)<cost(game))return false;visit.boneIds=[];visit.cycle++; }
    else { if(visit.easyUsed||game.lives!==1)return false;visit.easyUsed=true; }
    visit.visits++;
    game.entities.thor=null;
    game.thorRescue={time:0,before:game.lives,healed:false};
    game.thorNotice=undefined;game.thorBoneNotice=undefined;
    GameInput.clear();
  ...
  function request(game:Farm.GameState):boolean { return canCall(game)&&startRescue(game); }
  function landing(game:Farm.GameState):Farm.Point|null {
    const player=game.entities.chicken;
    for(const radius of [54,36,76,96,0])for(let i=0;i<12;i++) {
      ...
      if(at.x<24||at.y<24||at.x>WORLD.width-24||at.y>WORLD.height-24||!WildlifeRules.clear(at,at,HITBOX)||
        EnvironmentSystem.surfaceAt(game,{x:at.x,y:at.y+14})==='water'||!DetectionSystem.hasLineOfSight(getHitbox(player),at))continue;
      return at;
    }
    return null;
  }
  function deliver(game: Farm.GameState): void {
    const scene=game.thorRescue;
    if(!scene||scene.healed)return;
    scene.healed=true;
    ...
    if(at){const dog=make(at,at);dog.mode='greet';dog.timer=1.2;dog.age=2;game.entities.thor=dog;}
  ...
  function finishRescue(game:Farm.GameState):void {
    if(!game.thorRescue)return;
    deliver(game);game.thorRescue=undefined;GameInput.clear();GameManager.save(game);
  }
  function skip(game:Farm.GameState):boolean {
    if(game.phase!=='playing'||!game.thorRescue||game.thorRescue.time<.6)return false;
    finishRescue(game);return true;
  }
  ...
  function update(game: Farm.GameState, dt: number): void {
    ...
    if(game.thorRescue){
      ...
      if(game.thorRescue.time>=healAt)deliver(game);
      if(game.thorRescue.time>=duration)finishRescue(game);
      return;
    }
    if(game.thorNotice)game.thorNotice.time=Math.max(0,game.thorNotice.time-dt);
    if(game.thorBoneNotice)game.thorBoneNotice.time=Math.max(0,game.thorBoneNotice.time-dt);
    if(!cost(game)&&!game.thorVisit!.easyUsed&&game.lives===1&&startRescue(game))return;
    if(game.lake?.active)return;
    collectBones(game);
    ...
    if(dog.mode==='enter'){
      dog.mode='leave';dog.timer=14;dog.route=[];dog.routeTimer=0;
    ...
    dog.moving=Math.hypot(dog.vx,dog.vy)>1;dog.anim=CharacterArt.advance(dog.anim,'thor',distance(before,dog),{speed:dt>0?distance(before,dog)/dt:0});
  ...
  function restore(game: Farm.GameState, saved: Farm.ThorSnapshot | undefined): void {
    ...
    const allowed=(id:unknown):id is string=>{
