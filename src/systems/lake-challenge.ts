...
const LakeChallenge = (() => {
  ...
  const sheltered = new WeakSet<Farm.GameState>();
  ...
  // Compatibility with older callers: winning the challenge no longer unlocks
  // geographical protection. Only the active encounter temporarily suspends the wolf.
  function sanctuary(): Farm.Rect {
    const p = STRUCTURES.pond;
    return { x: p.x - 70, y: p.y - 70, w: p.w + 140, h: p.h + 140 };
  }
  function inSanctuary(_game: Farm.GameState): boolean { return false; }
  function safeWolfPosition(game: Farm.GameState): Farm.Point | null {
    const wolf = game.entities.wolf, radius = wolf.hitbox.r + 8, origin = game.entities.goose?.home;
    if (!origin) return null;
    const clear = (p: Farm.Point): boolean => {
      const q = { x: p.x + wolf.hitbox.ox, y: p.y + wolf.hitbox.oy };
      return distance(p, origin) > ARENA + 90 && q.x > radius && q.y > radius &&
        q.x < WORLD.width - radius && q.y < WORLD.height - radius &&
        OBSTACLES.every(r => r.blocking === false ||
          Math.hypot(q.x - clamp(q.x, r.x, r.x + r.w), q.y - clamp(q.y, r.y, r.y + r.h)) > radius);
    };
    if (clear(wolf)) return copy(wolf);
    const candidates: Farm.Point[] = [];
    for (const radius of [440, 520, 640]) for (let i = 0; i < 32; i++) {
  ...
  function start(game: Farm.GameState): boolean {
    if (!available(game) || ThorSystem.active(game)) return false;
    const resting = safeWolfPosition(game);
    if (!resting) return false;
    if (!game.lake) initialize(game);
    ...
    WolfAI.initialize(game); wolf.x = resting.x; wolf.y = resting.y; wolf.vx = 0; wolf.vy = 0;
    const goose = game.entities.goose!;
    goose.mode = 'recover'; goose.grace = .8;
    goose.cooldown = .8; goose.chargeHit = false; goose.chargeCounted=false; goose.timer = .6;
    goose.comboRemaining=0;goose.comboFollowup=false;goose.challengeFeinted=false;goose.earlyDodge=false;
  ...
  function releaseWolf(game: Farm.GameState): void {
  ...
  function cancel(game: Farm.GameState): boolean {
    if (!game.lake?.active) return false;
    game.lake.active = false; game.lake.misses = 0; game.lake.notice = 0;game.lake.counterWindow=0;
    const goose = game.entities.goose;
    if (goose) { goose.mode = 'return'; goose.timer = .8; goose.grace = 2; }
    releaseWolf(game);
    setStatus('Desafio interrompido. PANTO vai treinar a cara de bravo. Volte ao lago para tentar de novo!');
    GameManager.save(game); updateUI(game); return true;
  }
  function update(game: Farm.GameState, dt: number): void {
    if (game.phase !== 'playing' || !game.lake || !Number.isFinite(dt) || dt <= 0) return;
    game.lake.notice = Math.max(0, game.lake.notice - dt);
    if(game.lake.active&&SwimmingSystem.profile(game).swimming) {
      cancel(game);setStatus('Banho não vale carimbo! PANTO espera na margem para começar outra tentativa.');return;
    }
    if(game.lake.active&&(game.lake.counterWindow||0)>0){
      game.lake.counterWindow=Math.max(0,game.lake.counterWindow!-dt);
      if(game.lake.counterWindow===0){game.lake.feedback='expired';game.lake.notice=2.5;if(game.entities.goose)game.entities.goose.challengeFeinted=false;}
    }
    if (game.lake.active && (!game.entities.goose || distance(game.entities.chicken, game.entities.goose.home) > ARENA)) cancel(game);
  }
  function recordMiss(game: Farm.GameState, goose: Farm.Goose): boolean {
    ...
    updateUI(game);return true;
  }
  function canCounter(game: Farm.GameState): boolean {
    const goose=game.entities.goose,chicken=game.entities.chicken;
  ...
  function interact(game: Farm.GameState): boolean {
    if(game.phase!=='playing'||!game.lake?.active)return false;
    // Own the interaction during an attempt: a missed counter must not hide the player.
    if(!canCounter(game))return true;
    const lake=game.lake,goose=game.entities.goose!;
    lake.counterWindow=0;lake.misses=Math.min(3,lake.misses+1);lake.feedback='counter';lake.notice=2.5;
    goose.challengeFeinted=false;goose.comboRemaining=0;goose.comboFollowup=false;
    spawnBurst(goose.x, goose.y, '#f5df95', 12);
    if (lake.misses === 3) {
      lake.completed = true; lake.active = false; lake.notice = 7;
      goose.mode = 'defeated'; goose.moving = false; goose.vx = 0; goose.vy = 0;
      SkinSystem.unlockLake(game);
      buildObstacles(game);
      releaseWolf(game);
      GooseSystem.rescue(game);
      AudioSystem.play('panto-victory', { volume: .85 });
      setStatus('PANTO resgatado! +100 pontos e mais um amigo no poleiro. Ponte liberada! O lobo continua à solta pela fazenda.', 'win');
    } else {
  ...
  function recordHit(game: Farm.GameState): void {
    const lake=game.lake;if(!lake?.active)return;
    lake.attempts+=1;lake.counterWindow=0;lake.feedback='hit';lake.notice=2.5;
    const goose=game.entities.goose!;goose.comboRemaining=0;goose.comboFollowup=false;goose.challengeFeinted=false;
    if(lake.attempts>=3){
      lake.active=false;lake.misses=0;lake.feedback='failed';lake.notice=6;
      goose.mode='recover';goose.timer=1;goose.grace=2;releaseWolf(game);
      setStatus('Três bicadas: tentativa encerrada. PANTO reteve seu crachá. Volte e desafie de novo!');
    }else setStatus(`Pegou! ${3-lake.attempts} ${lake.attempts===2?'chance restante':'chances restantes'}. O carimbo desta rodada ainda está com PANTO.`);
    GameManager.save(game);updateUI(game);
  }
  function blocksWolf(game: Farm.GameState): boolean {
    return game.lake?.active === true && game.lake.completed !== true;
  }
  ...
  function pondObstacles(game?: Farm.GameState | null): Farm.Obstacle[] {
    const p = STRUCTURES.pond;
    const water = { x: p.x+20, y: p.y+20, w: p.w-40, h: p.h-40, type: 'pond' };
    if (!game?.lake?.completed || game.worldSeed !== WORLD.layout?.seed) return [water];
    const b = bridge();
  ...
  function drawGround(game: Farm.GameState): void {
    ...
    if (p.x < -b.w || p.x > canvas.width || p.y < -b.h || p.y > canvas.height) return;
    ctx.save(); ctx.translate(Math.round(p.x),Math.round(p.y));ctx.imageSmoothingEnabled=false;
    const rect=(x:number,y:number,w:number,h:number,color:string):void=>{ctx.fillStyle=color;ctx.fillRect(Math.round(x),Math.round(y),Math.round(w),Math.round(h));};
    ...
    for(const end of [0,b.w]){
      ctx.fillStyle='#bca06c55';ctx.beginPath();ctx.ellipse(end+(end?12:-12),b.h/2,34,25,0,0,Math.PI*2);ctx.fill();
      ctx.fillStyle='#c5a977';ctx.beginPath();ctx.ellipse(end+(end?10:-10),b.h/2,25,19,0,0,Math.PI*2);ctx.fill();
      for(let i=0;i<18;i++)rect(end+(end?1:-1)*(8+(i*13)%23),b.h/2-12+(i*17)%25,2,2,i%3?'#dfbd7a55':'#93784e55');
    }
    // Abutments meet the left and right banks without extending toward the fence.
    for(const end of [0,b.w]){
      ctx.fillStyle='#bca06c55';ctx.beginPath();ctx.ellipse(end,b.h/2,20,47,0,0,Math.PI*2);ctx.fill();
      for(let i=0;i<5;i++){
        const x=end-5,y=-5+i*17;
        rect(x+3,y+6,12,16,'#46514050');rect(x,y,11,16,i%2?'#a9a78a':'#b9b499');
        rect(x+1,y,9,2,'#d0c8a8');rect(x+8,y+3,3,12,'#7e806b');
      }
    }
    if(open){
      rect(4,10,b.w,b.h-3,'#254f4235');
      rect(0,2,b.w,7,'#62412a');rect(0,b.h-9,b.w,7,'#62412a');
    }
    const count=Math.ceil(b.w/14),step=b.w/count;
    for(let i=0;i<count;i++){
      if(!open&&i>1&&i<count-2)continue;
      const x=i*step;
      rect(x,4,step+1,b.h-6,'#66442c');
      rect(x,4,step-2,b.h-10,['#b4854b','#b98b51','#aa7943','#c19256'][i%4]);
      rect(x+1,4,step-4,2,'#dbaf70');rect(x+step-4,7,1,b.h-16,'#946135');
      rect(x+5,16+(i*13)%23,1,18+(i%3)*5,'#86552e55');
      rect(x+4,9,2,2,'#665844');rect(x+4,b.h-12,2,2,'#665844');
    }
    ...
    for(const y of [3,b.h-4]){
      if(open)for(let i=1;i<postXs.length;i++){
        const a=postXs[i-1],z=postXs[i];
        ctx.lineWidth=3;ctx.strokeStyle='#705638';ctx.beginPath();ctx.moveTo(a+3,y-14);ctx.quadraticCurveTo((a+z)/2,y-6,z+3,y-14);ctx.stroke();
        ctx.lineWidth=1;ctx.strokeStyle='#d3b57b';ctx.stroke();
      }
      for(const x of postXs){
        rect(x+4,y-4,11,8,'#263e3530');rect(x,y-20,7,25,'#61432b');
        rect(x+1,y-18,4,20,'#ae7c43');rect(x+1,y-18,1,20,'#d0a260');
        rect(x-1,y-23,9,5,'#795332');rect(x,y-23,7,2,'#d4ab6c');
      }
    }
    if(!open)for(const x of [11,b.w-11]){
      ctx.strokeStyle='#d8b77b';ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(x,-11);ctx.quadraticCurveTo(x+5,b.h/2-14,x,b.h-18);ctx.stroke();
      const y=b.h/2-18;
      rect(x-13,y,26,18,'#805532');rect(x-11,y+2,22,14,'#bb8b4e');
      ctx.strokeStyle='#efd28e';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(x-4,y+5);ctx.lineTo(x+4,y+12);ctx.moveTo(x+4,y+5);ctx.lineTo(x-4,y+12);ctx.stroke();
    }
    ctx.restore();
  }
  function updateUI(game: Farm.GameState): void {
    ...
    if(hud&&value&&hudTitle&&hudCue){
      ...
      hudCue.textContent=completed?'Panto resgatado! Ponte aberta. Cuidado: o lobo continua por perto.':failed?'Três bicadas. Tente o desafio novamente.':cue;
      ...
      const timer=document.getElementById('lakeWindow') as HTMLProgressElement|null;
      if(timer){timer.hidden=!counter;timer.max=1;timer.value=(lake?.counterWindow||0)/(lake?.counterDuration||3);}
      for(let i=1;i<=3;i++){
        const stamp=document.getElementById(`lakeStamp${i}`);
        if(stamp){stamp.dataset.earned=String(i<=(game.lake?.misses||0));stamp.textContent=i<=(game.lake?.misses||0)?'✓':['BOTE','DUPLO','BLEFE'][i-1];}
      }
    }
    title.textContent = completed ? 'PANTO liberou a ponte' : 'PANTO · O fiscal do lago';
    text.textContent = completed ? 'Ponte aberta e Panto no poleiro. A lagoa não protege contra o lobo.' : active ?
