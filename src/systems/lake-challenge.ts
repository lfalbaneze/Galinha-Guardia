...
const LakeChallenge = (() => {
  ...
  function cancel(game: Farm.GameState): boolean {
    ...
    game.lake.active = false; game.lake.misses = 0; game.lake.notice = 0;
    const goose = game.entities.goose;
  ...
  function recordMiss(game: Farm.GameState, goose: Farm.Goose): boolean {
    ...
    lake.misses = Math.min(3, lake.misses + 1); lake.notice = 2.5;
    ...
    } else setStatus(`Panto errou! ${lake.misses}/3 investidas desviadas. Espere o próximo aviso.`, 'win');
    GameManager.save(game); updateUI(game); return true;
  }
  ...
  function drawGround(game: Farm.GameState): void {
    ...
    if (open) { ctx.fillStyle = '#254c3433'; ctx.fillRect(Math.round(p.x+5), Math.round(p.y+6), b.w, b.h); }
    const plank = (y: number): void => {
      ctx.fillStyle = '#573e29'; ctx.fillRect(p.x,y,b.w,11);
      ctx.fillStyle = (Math.round(y-p.y)/12)%2 ? '#ba9055' : '#c7a264'; ctx.fillRect(p.x+2,y,b.w-4,8);
      ctx.fillStyle = '#e2c18a'; ctx.fillRect(p.x+2,y,b.w-4,1);
      ctx.fillStyle = '#635437'; ctx.fillRect(p.x+7,y+4,2,2);ctx.fillRect(p.x+b.w-9,y+4,2,2);
    };
    for(let y=0;y<b.h;y+=12) if(open || y<30 || y>b.h-36) plank(Math.round(p.y+y));
    if(open) {
      ctx.fillStyle='#765339';ctx.fillRect(p.x+1,p.y,3,b.h);ctx.fillRect(p.x+b.w-4,p.y,3,b.h);
    } else {
      for (const y of [p.y+22,p.y+b.h-25]) {
        ctx.fillStyle='#695035';ctx.fillRect(p.x-2,y-18,6,24);ctx.fillRect(p.x+b.w-4,y-18,6,24);
        ctx.strokeStyle='#d3b077';ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(p.x+2,y-12);ctx.quadraticCurveTo(p.x+b.w/2,y-3,p.x+b.w-2,y-12);ctx.stroke();
      }
    }
    ctx.restore();
  }
