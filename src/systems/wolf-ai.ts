...
const WolfAI = (() => {
  ...
  function refugeContains(point: Farm.Point | Farm.Body): boolean {
    return typeof FarmRefuge!=='undefined'&&FarmRefuge.contains(point);
  }
  function refugeBounds(): Farm.Rect | null {
  ...
  function updateTracks(game: Farm.GameState,dt:number): void {
    ...
    if(refugeContains(c)||c.hidden||!c.sprinting||c.sneaking||EnvironmentSystem.surfaceAt(game,{x:c.x,y:c.y+14})==='water'){trail.last=null;return;}
  ...
  function update(game: Farm.GameState, dt: number): void {
    ...
    if(refugeSafe) {
      ...
      if(retreating) {
        ...
        wolf.speechTime=2.6;wolf.speechPriority=2.6;
