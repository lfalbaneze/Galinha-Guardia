...
const WildlifeRules = (() => {
  ...
  function validPoint(p: unknown): p is Farm.Point {
    if (!p || typeof p !== 'object') return false;
    const v = p as Farm.Point;
    return Number.isFinite(v.x) && Number.isFinite(v.y) && v.x >= 0 && v.y >= 0 && v.x <= WORLD.width && v.y <= WORLD.height;
  }
  function rank(id: string, salt = 0): number {
    let value = (WORLD.layout.seed ^ salt) >>> 0;
    for (const letter of id) value = Math.imul(value ^ letter.charCodeAt(0), 16777619) >>> 0;
    return (value ^ value >>> 16) >>> 0;
  }
  function pathDistance(p: Farm.Point): number {
    return WORLD.paths.reduce((d, r) => Math.min(d, Math.hypot(p.x-clamp(p.x,r.x,r.x+r.w), p.y-clamp(p.y,r.y,r.y+r.h))), Infinity);
  }
  function reserved(game: Farm.GameState, p: Farm.Point, margin = 110): boolean {
  ...
  function clear(from: Farm.Point, to: Farm.Point, box: Farm.Hitbox, obstacles = OBSTACLES): boolean {
    const a = {x:from.x+box.ox,y:from.y+box.oy}, b = {x:to.x+box.ox,y:to.y+box.oy}, r=box.r+.25;
    if (!validPoint(b) || b.x<r || b.y<r || b.x>WORLD.width-r || b.y>WORLD.height-r) return false;
    return obstacles.every(o=>o.blocking===false || !DetectionSystem.segmentIntersectsRect(a,b,{x:o.x-r,y:o.y-r,w:o.w+2*r,h:o.h+2*r}));
  }
  function onScreen(p: Farm.Point, margin = 20): boolean {
  ...
  function observe(game: Farm.GameState, origin: Farm.Point, range: number, obstacles=OBSTACLES): boolean {
  ...
  function move(body: Farm.Entity, target: Farm.Point, amount: number, onStep?: ()=>boolean): 'arrived'|'moving'|'blocked'|'hit' {
    const dx=target.x-body.x, dy=target.y-body.y, length=Math.hypot(dx,dy);
    if (length<.01) return 'arrived';
    ...
    for(let i=0;i<steps;i++) {
      ...
      if(!clear(body,next,body.hitbox))return 'blocked';
      body.x=next.x;body.y=next.y;
      if(onStep?.())return 'hit';
    }
