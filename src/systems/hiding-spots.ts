...
const HidingSpots = (() => {
  let spots: Farm.Cover[] = [];
  function initialize(layout: Farm.Layout = WORLD.layout): void {
  ...
  function contains(spot: Farm.Cover, chicken: Farm.Point): boolean {
  ...
  function bonusInReach(chicken: Farm.Chicken, chick: Farm.Animal): boolean {
    const spot = spots.find(s => s.id === chick.coverId);
    if (!spot || !hasBonusClue(chicken, chick)) return false;
  ...
  function toggle(game: Farm.GameState): void {
    if (game.phase !== "playing") return;
    if(SwimmingSystem.profile(game).swimming) {
      setStatus('Boia não é moita! Volte à margem para procurar um esconderijo.');return;
    }
  ...
  function update(game: Farm.GameState, dt = 0): void {
  ...
  function restore(game: Farm.GameState, saved: Farm.ChickenSnapshot): void {
  ...
  function drawForeground(game: Farm.GameState): void {
    const chicken = game.entities.chicken;
    if (!chicken.hidden && (chicken.hideBlend || 0) < 0.02) return;
    const spot = spots.find(s => s.id === chicken.hidingSpotId) || candidate(chicken);
    if (spot) FarmArt.drawCoverForeground(ctx, spot, camera, Math.max(0.35, chicken.hideBlend || 0), chicken, game);
  }
  function pill(x: number, y: number, width: number, title: string, exposed = false): void {
    ctx.save();ctx.font='bold 11px Trebuchet MS, sans-serif';
    const measured=ctx.measureText(title).width;
    if(Number.isFinite(measured))width=Math.min(width,Math.ceil(measured)+20);
    ...
    ctx.strokeStyle = exposed ? '#f6b08a' : '#c8d89a66';ctx.lineWidth=1;
    ctx.beginPath();ctx.roundRect(-width/2,0,width,24,6);ctx.fill();ctx.stroke();
    ctx.fillStyle='#fff4cf';ctx.textAlign='center';
  ...
  function drawIndicators(game: Farm.GameState): void {
    ...
    if (spot && !(spot.type === 'hay' && (chicken.hidden || (chicken.hideBlend || 0) > 0.02))) {
      ctx.save(); ctx.strokeStyle = exposed || blocked ? "#ff956c" : chicken.hidden ? "#e6f6be" : "#fff6bf";
    ...
    if (blocked) {
      ...
      if (!celebrating || exposed) pill(p.x, p.y - 73, 185,
        exposed ? "Ele viu você! Saia daí!" : control('exit')==='Sair'?'Escondida · sair':`Escondida · ${control('exit')} para sair`, exposed);
    } else if (spot) {
      if (!celebrating) pill(p.x,p.y-73,155,control('hide')==='Esconder'?'Esconderijo':`${control('hide')} · esconder`);
