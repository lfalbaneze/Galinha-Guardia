...
const FoxSystem = (() => {
  ...
  function denLabel(fox: Farm.Fox | null): string {
    return fox?.name==='Amanda'?'Moita da Amanda':fox?'Moita do Lorenzo':'Moita da raposa';
  }
  function say(fox: Farm.Fox, event: keyof typeof lines.Lorenzo): void {
    const choices=lines[fox.name||'Lorenzo'][event],index=fox.speechCounts[event]||0;
    fox.speech=choices[index%choices.length];fox.speechCounts[event]=index+1;
    fox.speechTime=3;fox.speechCooldown=8;
  }
  function getConfig(game: Farm.GameState): {warning:number;speed:number;rest:number;cooldown:number;relocateEvery:number;travelSpeed:number} {
  ...
  function initialize(game: Farm.GameState): void {
    ...
    for(const c of candidates){
      if(chosen.every(p=>distance(p.home,c.home)>390)) chosen.push(c);
  ...
  function routeTo(fox: Farm.Fox, home: Farm.Point): Farm.Point[] {
    ...
    if(distance(route[route.length-1],home)>.01)route.push(home);
  ...
  function plan(fox: Farm.Fox, target: Farm.Point): Farm.Point {
    ...
    for(let s=5;s<=LEASH;s+=5){
      ...
      if(!WildlifeRules.clear(end,next,fox.hitbox) || distance(fox.home,next)>LEASH)break;
      end=next;
    }
    return end;
  }
  ...
  function bump(game: Farm.GameState,fox: Farm.Fox): boolean {
    ...
    if(game.phase!=='playing'||game.lake?.active||fox.mode!=='dash'||fox.hit||c.hidden||c.invulnerable>0||
      !circleVsCircle(fox,c)||!DetectionSystem.hasLineOfSight(getHitbox(fox),getHitbox(c)))return false;
    ...
    if(game.lives===0)finishLose(`${fox.name} levou essa! Tentar novamente começa do zero. Na próxima, desvie para o lado da investida.`);
    else setStatus(`${fox.name} cobrou um coração! Restam ${game.lives}. Desvie para o lado e aproveite a pausa da raposa.`);
    refreshHud();
  ...
  function update(game: Farm.GameState,dt: number): void {
    ...
    for(const fox of game.entities.foxes||[]){
      ...
      fox.speechTime=Math.max(0,fox.speechTime-dt);fox.speechCooldown=Math.max(0,fox.speechCooldown-dt);
      ...
      if(fox.mode==='hidden'){
        ...
        if(!WildlifeRules.observe(game,getHitbox(fox),LEASH+45)){
          fox.mode='hidden';fox.cooldown=1.5;
        }else if(fox.timer<=0){fox.mode='dash';fox.timer=LEASH/config.speed+.1;fox.attempts++;}
        ...
        if(!WildlifeRules.clear(fox,target,fox.hitbox)){
          if(!fox.route.length && fox.timer<=0){fox.route=routeTo(fox,fox.home);fox.timer=1;}
        ...
        if(moved==='arrived'){
          if(distance(fox,fox.home)<1){fox.mode='hidden';fox.hit=false;fox.cooldown=Math.max(1,fox.cooldown);fox.route=[];}
          else fox.route.shift();
        }
      }
  ...
  function drawWarnings(game: Farm.GameState): void {
    ...
    for(const fox of game.entities.foxes||[]){
      if(visible(game,fox)){
        ...
        ctx.strokeStyle='#243c2ddd';ctx.lineWidth=3;ctx.lineJoin='round';ctx.strokeText(fox.name,x,y);
        ctx.fillStyle=fox.name==='Amanda'?'#ffc5df':'#fff0bd';ctx.fillText(fox.name,x,y);ctx.restore();
      }
      if(fox.speechTime>0&&fox.speech&&visible(game,fox)){
        ...
        ctx.fillStyle=fox.name==='Amanda'?'#ffe0eefa':'#fff0cefa';ctx.strokeStyle='#523c35';ctx.lineWidth=1.5;
        ctx.beginPath();ctx.roundRect(x-width/2,y-18,width,29,7);ctx.fill();ctx.stroke();
        ctx.fillStyle='#50382f';ctx.fillText(fox.speech,x,y+1,width-16);ctx.restore();
      }
  ...
  function snapshot(game: Farm.GameState): Farm.FoxSnapshot[] {
  ...
  function restore(game: Farm.GameState, saved: unknown): void {
    ...
    const candidates=dens(game),claimed=new Set<string>();
    const placements=new Map<string,typeof candidates[number]>();
    // Reserve saved homes together so two foxes can exchange their original dens.
    if(getConfig(game).relocateEvery)for(const fox of game.entities.foxes||[]){
      ...
      if(!den || !WildlifeRules.validPoint(s) || !WildlifeRules.clear(s,s,fox.hitbox) ||
        !routeTo({...fox,...point(s)},den.home).length)continue;
      placements.set(fox.id,den);claimed.add(den.spot.id);
    }
    for(const fox of game.entities.foxes||[]){
      const den=placements.get(fox.id);
      if(den){fox.bushId=den.spot.id;fox.home=point(den.home);}
      else if(fox.bushId && claimed.has(fox.bushId)){
        const fallback=candidates.find(c=>!claimed.has(c.spot.id));
        if(fallback){fox.bushId=fallback.spot.id;fox.home=point(fallback.home);}
      }
      if(fox.bushId)claimed.add(fox.bushId);
      Object.assign(fox,point(fox.home));
      const s=saved.find((v: unknown)=>v && typeof v==='object' && (v as Farm.FoxSnapshot).id===fox.id);
      if(!WildlifeRules.validPoint(s)||!WildlifeRules.clear(s,s,fox.hitbox))continue;
      const record=s as Partial<Farm.FoxSnapshot>;
      if(!den && (distance(s,fox.home)>LEASH||!WildlifeRules.clear(fox.home,s,fox.hitbox)))continue;
