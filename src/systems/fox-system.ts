...
const FoxSystem = (() => {
  ...
  function getConfig(game: Farm.GameState): {warning:number;speed:number;rest:number;cooldown:number} {
  ...
  function drawWarnings(game: Farm.GameState): void {
    ...
    for(const fox of game.entities.foxes||[]){
      ...
      ctx.fillStyle='#312d2b';ctx.fillRect(Math.round(x-12+shake),y-30,24,8);
      ctx.fillStyle='#f7d99a';ctx.fillRect(Math.round(x-8+shake),y-28,4,3);ctx.fillRect(Math.round(x+4+shake),y-28,4,3);
      ctx.restore();
    }
  }
