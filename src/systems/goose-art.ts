...
const GooseArt = (() => {
  ...
  const threatening=(g:Farm.Goose)=>['notice','warning','feint'].includes(g.mode);
  let pending:Promise<boolean>|null=null;
  function load(loader?:(src:string)=>Promise<CanvasImageSource>):Promise<boolean>{
    if(pending)return pending;
    pending=Promise.all([sheet.load(loader),alarmSheet.load(loader)]).then(r=>{pending=null;return r.every(Boolean);});return pending;
  }
  ...
  function draw(c:CanvasRenderingContext2D,goose:Farm.Goose,view:Farm.Camera):boolean{
    const active=threatening(goose)?alarmSheet:sheet;if(!active.ready)return false;
    ...
    else active.drawFrame(c,x,y+14,f.row,f.column,1);
    if(goose.mode==='stunned'){
      c.fillStyle='#e8c65a';for(let i=0;i<3;i++){
        ...
        c.fillRect(sx-3,sy-1,7,3);c.fillRect(sx-1,sy-3,3,7);
      }
    }else if(['notice','warning','charge','feint'].includes(goose.mode)){
      c.font='bold 17px Trebuchet MS,sans-serif';c.textAlign='center';c.fillStyle='#fff0b3';
      c.fillText(goose.mode==='notice'?'?':'!',x,y-58);
    }
    c.restore();return true;
  }
  return {draw,frameFor,source,width,height,frames,columns,scale,load,
    install:(loader:(src:string)=>CanvasImageSource)=>{sheet.install(loader);alarmSheet.install(loader);},
