...
const ScarecrowArt = (() => {
  ...
  function drawPost(c:CanvasRenderingContext2D,s:Farm.Scarecrow,view:Farm.Camera):void {
    ...
    active.drawFrame(c,x,y,0,InterfaceMotion.reduced?0:Math.floor(s.clock*4)%columns,1);c.restore();
  }
  function drawCrow(c:CanvasRenderingContext2D,b:Farm.Crow,index:number,clock:number,view:Farm.Camera,scared=false):void {
    ...
    c.save();c.globalAlpha*=b.opacity;active.drawFrame(c,x,y,rows[direction],column,1,b.z);c.restore();
  }
  return {drawPost,drawCrow,frames:post.frames,
    load:(loader?:(src:string)=>Promise<CanvasImageSource>)=>Promise.all(sheets.map(s=>s.load(loader))).then(r=>r.every(Boolean)),
    install:(loader:(src:string)=>CanvasImageSource)=>{sheets.forEach(s=>s.install(loader));},
