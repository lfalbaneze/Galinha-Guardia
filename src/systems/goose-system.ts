...
const GooseSystem = (() => {
  ...
  function warn(game: Farm.GameState, goose: Farm.Goose, config: Farm.GooseConfig): void {
    goose.mode = 'warning'; goose.timer = config.warning; goose.anchor = point(goose);
    const chicken = game.entities.chicken;
  ...
  function update(game: Farm.GameState, dt: number): void {
    ...
    if (goose.mode === 'patrol') {
      if (goose.cooldown <= 0 && canNotice(game, goose, config)) warn(game, goose, config);
      ...
      if (!peck(game, goose, config)) {
        ...
        if (goose.mode === 'charge' && (arrived || goose.timer <= 0 || distance(before, goose) < .01)) recover(goose, config);
      }
  ...
  function restore(game: Farm.GameState, saved: unknown): void {
    ...
    goose.mode = 'return'; goose.grace = 2;
  }

  function drawTerritory(game: Farm.GameState): void {
    const goose = game.entities.goose;
    if (!goose || game.phase !== 'playing' || distance(game.entities.chicken, goose.home) > 500) return;
    const p = worldToScreen(goose.home);
    ctx.save(); ctx.strokeStyle = '#e8d69965'; ctx.lineWidth = 1.5; ctx.setLineDash([7, 12]);
    ctx.beginPath(); ctx.arc(p.x, p.y + OFFSET_Y, TERRITORY, 0, Math.PI * 2); ctx.stroke();
  ...
  function drawIndicator(game: Farm.GameState): void {
    ...
    if (goose.mode === 'warning' || goose.mode === 'charge' || goose.notice > 0) {
      ctx.save(); const x = clamp(p.x, 80, canvas.width - 80), y = Math.max(42, p.y - 76);
      ctx.fillStyle = '#503b27'; ctx.beginPath(); ctx.roundRect(x - 67, y - 19, 134, 28, 5); ctx.fill();
