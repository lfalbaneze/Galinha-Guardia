...
const DetectionSystem = (() => {
  ...
  function segmentIntersectsRect(from: Farm.Point, to: Farm.Point, rect: Farm.Rect): boolean {
    let enter = 0;
    let leave = 1;
  ...
  function hasLineOfSight(from: Farm.Point, to: Farm.Point, obstacles: readonly Farm.Obstacle[] = OBSTACLES): boolean {
  ...
  function canSee(wolf: Farm.Wolf, chicken: Farm.Chicken, options: Partial<Farm.DetectionConfig> = {}): boolean {
    ...
    if (distance > config.range && distance > config.closeRange) return false;
    if (!hasLineOfSight(wolf, chicken)) return false;
  ...
  function hear(wolf: Farm.Wolf, chicken: Farm.Chicken, config: Farm.DetectionConfig): Farm.Point | null {
  ...
  function perceive(wolf: Farm.Wolf, chicken: Farm.Chicken, options: Partial<Farm.DetectionConfig> = {}): Farm.Perception {
    const config = { ...defaults, ...options };
    ...
    return { visible, distance, contact: visible && distance <= config.contactRange,
      ...
      heardPoint: hear(wolf, chicken, config) } as Farm.Perception;
  }
