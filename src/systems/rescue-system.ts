const RescueSystem = {
  ...
  safePosition(index: number): Farm.FriendSnapshot {
    return FarmRefuge.home(index);
  },
  chickPosition(index: number): Farm.FriendSnapshot { return FarmRefuge.home(index, true); },
  knowsSecret(game: Farm.GameState): boolean {
    return game.rescuedChicks > 0 || game.entities.chicks.some(c => c.discovered);
  },
  isSecret(animal: Farm.Animal): boolean { return animal.type === "chick" && !animal.rescued && !animal.discovered; },
  secretHint(game: Farm.GameState): Farm.Animal | null {
    if (game.phase !== "playing" || game.lake?.active) return null;
  ...
  callTarget(game: Farm.GameState): Farm.Animal | null {
    if (game.phase !== 'playing' || game.lake?.active || WolfAI.isExposed(game)) return null;
    const chicken = game.entities.chicken;
  ...
  callChick(game: Farm.GameState): boolean {
  ...
  inRescueReach(chicken: Farm.Chicken, animal: Farm.Animal): boolean {
  ...
  flee(game: Farm.GameState, animal: Farm.Animal, home: Farm.Point, area: Farm.Area, dt: number): void {
    ...
    let best: (Farm.Point & { heading: number }) | null = null, score = -Infinity;
  ...
  wander(animal: Farm.Animal, home: Farm.Point, area: Farm.Area): void {
  ...
  refugeWander(animal: Farm.Animal, home: Farm.Point, index: number): void {
    ...
    for(let attempt=0;attempt<8;attempt++) {
      ...
      if(distance(probe,target)>2)continue;
      animal.targetX=target.x;animal.targetY=target.y;return;
    }
    animal.targetX=home.x;animal.targetY=home.y;
    ...
    for (const animal of RescueSystem.all(game)) {
      ...
      if (!animal.rescued) {
        ...
        if (calm || (relieved && !threat)) {
          animal.fleeFrom = null; animal.fleeTime = 0; animal.fleeHeading = null;
        }
        ...
        if (fleeing) {
          ...
          if (gap > 1) Player.move(animal, dx / gap * step, dy / gap * step);
          animal.targetX = animal.x; animal.targetY = animal.y;
          animal.speechTime = 0;
        ...
        if ((touchedBeforeMove || RescueSystem.inRescueReach(chicken, animal)) && GameManager.rescue(game, animal)) {
          ...
          animal.restTime = .8 + (index % 4) * .18; animal.wanderTime = 0;
          GameManager.save(game);
          refreshHud();
        }
      } else if (chick) {
        const safe=safePosition(index);
        animal.x=safe.x;animal.y=safe.y;animal.targetX=safe.x;animal.targetY=safe.y;
        ...
        animal.temper='safe';animal.restTime=Math.max(0,(animal.restTime||0)-dt);
        ...
        if(len<=4&&animal.restTime<=0&&dt>0) {
          ...
          dx=animal.targetX-animal.x;dy=animal.targetY-animal.y;len=Math.hypot(dx,dy);
        }
        if(len>4&&animal.restTime<=0&&dt>0) {
          const active=['rabbit','dog','cat','duck'].includes(animal.species);
          const heavy=['cow','donkey','horse'].includes(animal.species);
          ...
          Player.move(animal,dx/len*step,dy/len*step);resolveEnvironment(animal);
          ...
          if(distance(animal,{x:animal.targetX,y:animal.targetY})<=4) {
            ...
            animal.targetX=animal.x;animal.targetY=animal.y;animal.restTime=.8;
          }
        }
      }
