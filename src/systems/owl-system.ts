...
const OwlSystem = (() => {
  ...
  function canSee(owl: Farm.Owl,chicken: Farm.Chicken): boolean {
    if(chicken.hidden||chicken.invulnerable>0||distance(owl.perch,chicken)>owl.range)return false;
    ...
    for(const owl of game.entities.owls||[]){
      ...
      if(canSee(owl,game.entities.chicken)&&visible(game,owl)){
        ...
        if(owl.alertProgress>=1){
          const heard=WolfAI.investigateSound(game,point(owl.target),360);
  ...
  function drawGround(game: Farm.GameState): void {
    ...
    for(const owl of game.entities.owls||[]){
      if(!visible(game,owl)||owl.mode==='cooldown')continue;
  ...
  function drawEntity(owl: Farm.Owl): void {
    if(!WildlifeRules.onScreen(owl.perch,110))return;
